...
def get_case_from_id(id: str, cases: list[Case]) -> Case:
    """
    Small util to search for a case based on a supplied case ID. Only used for Delta path handling
    """
...
def data_from_path(case: Case, path:str, cases: list[Case]=[]) -> Any:
    """
    Get data from a `Case` using a supplied path.

    Path is formatted as a string with '/' separators. Travels down the path by recursing through the `Case` object.
    Is able to find data as an attribute, in a dictionary or list, as well as other specific data structures.
    If data is a result then 'result' can be omitted from the path.
    If data is a value attribute then 'value' can be omitted from the path.
    """
    ...
    def _search_path(case: Case, component: str) -> Any:
        ...
        try:
            ...
            if isinstance(case, (int, float, str)):
                return case
...
class ReportItem(BaseModel):
    """Generic parent class for items in the report."""
...
class Report(BaseModel):
    """
    Store and assemble report items into a complete report.

    Additional control for case_by_case appendix inclusion and specifying output directory.
    Control over including section titles and new pages is available in the relevant `ReportItems`.
    """
    ...
    def _create_header_footer(self) -> PageStyle:
        """Control the header and footer components for each report page"""
    ...
    def create_pdf(self, filename:str, cases: list[Case], landscape:bool=False) -> None:
        """
        Assemble a pdf from automatically generated latex code for each item in the report.

        Creates boilerplate for the doc and then iterates through each `ReportItem.get_doc_item` in the report.
        """
        ...
        doc.packages.append(Package("caption")) # For more caption style controls
        ...
        # Setup geometry: page size, margins, and landscape controls
        ...
        # Date
        doc.append(Command('huge', NoEscape(r'\today')))  # Current date
        doc.append(NoEscape(r"\\"))

        # Image
        doc.append(Command('vspace*{1cm}'))
        doc.append(NoEscape(r'\includegraphics[width=0.6\textwidth]{' + os.path.join(os.path.dirname(__file__), 'img', 'flow360.png') + '}'))
        ...
        # doc.preamble.append(NoEscape(r'\renewcommand{\footrulewidth}{5.4pt}')) 
        ...
        # Manage file directory for output
        ...
        # Generate the PDF
        doc.generate_pdf(filepath=self.output_dir + "/" + filename, clean_tex=False)
        os.chdir(current_dir)


class Summary(ReportItem):
    """
    Wrapper for a Table setup that specifies all the case names in the report.

    Additionally includes a user-specified text string above the table to introduce the report.
    """
...
class Delta(BaseModel):
    """
    Compare two values as a taget case against a reference case. Returns the mean value of the difference.
    """
...
class Table(ReportItem):
    """
    Assemble a table of data from specified paths for each chosen case.

    Table includes each data_path value as a column and each case as a row.
    Custom headings can be supplied, otherwise the end of the `data_path` is used.
    Section title can be disabled using if `section_title` is set to `None`.
    """
    ...
    def get_doc_item(self, cases: List[Case], doc: Document, section_func: Union[Section, Subsection]=Section, case_by_case=False) -> None:
        # Control if table is on a new page
...
class Chart(ReportItem):
    """
    Parent class for the chart `ReportItem` objects.

    The charts can be assembled into rows of X charts using `items_in_row`. Use -1 to include all cases in one row.
    Specific cases can be plot by supplying a list of case IDs to `select_case_ids`.
    Section title can be disabled using if `section_title` is set to `None`.
    """
    section_title: Union[str, None]
    fig_name: str
    ...
    def _chart_latex_setup(self, doc: Document, cases: list[Case], section_func: Union[Section, Subsection]) -> None:
        """Handle standardised latex formating options for Chart objects."""
...
class Chart2D(Chart):
    """
    Create an individual chart from a 2-list `data_path` using each provided `Case`.

    First path is the X-axis, second is Y-axis.
    Axis lables come from the final part of the `data_path`.
    Chart is plot using standard `matplotlib` and saved as PNG files.
    File name is supplied by the user and the case name is appended.
    The name will include '_cbc_' if image is created when `case_by_case` is `True`. This is to prevent overwriting.
    All `Case` plots will be combined if `single_plot` is `True`.
    """
    ...
    def get_doc_item(self, cases: List[Case], doc: Document, section_func: Union[Section, Subsection]=Section, case_by_case=False) -> None:
        self._chart_latex_setup(doc, cases, section_func)
...
class Chart3D(Chart):
    """
    Create an image using UVF Shutter by calling the S3 instance.

    Uses asynchronous REST API call to request PNG data from the server for each selected `Case` object.
    Call has exponential backoff for maximum 300 seconds if receiving 503 server is busy responses.
    Will check if filename already exists and not make new request - prevents case_by_case unnecessarily requesting.
    Images can then be assembled into rows or displayed independently.
    """

    # Potential arguments for later versions of Shutter
    # field: str
    # camera: List[float]
    # limits: List[float]

    async def process_3d_images(self, cases: list[Case], fig_name: str, url: str, manifest: list) -> list[str]:
        # Handles 300 second exponential backoff if server is busy and returns 503 response
        ...
        async with aiohttp.ClientSession() as session:
            ...
            # Assemble task for each case
            for case in cases:
                # Stops case_by_case re-requesting images from the server
                # Empty tasks list doesn't cause issues with later asyncio.gather or zip
                ...
                tasks.append(task)
            # Request all task results asynchronously 
            responses = await asyncio.gather(*tasks)

            # Server response is png encoding that just needs to be written to file
    ...
    def get_doc_item(self, cases: List[Case], doc: Document, section_func: Union[Section, Subsection]=Section, case_by_case: bool=False):
        ...
        os.chdir("/home/matthew/Documents/uvf-shutter/server/src/manifests")
...
if __name__ == "__main__":
    ...
    # Finalise included values in Inputs table
    ...
    report = Report(
        ...
        output_dir="/home/matthew/Documents/Flow360/report_output"
    )
