...
*These settings have no control in the time-average probe panels. Configure them through the Python API.*
...
- **Options:** (choose one or more)
  ...
  - `"rms"`
>**Notes:**
> - A probe output configured in the WebUI always writes the mean. Set this through {py:attr}`~flow360.TimeAverageProbeOutput.statistics` or {py:attr}`~flow360.TimeAverageSurfaceProbeOutput.statistics`.
> - Requesting both writes the mean under each field's normal name and the RMS under `<field>_rms`.
> - The RMS is taken on the raw signal without subtracting the mean, so it is not a fluctuation intensity about the mean.
> - For probe outputs the RMS shares the mean's averaging window, including **Start step**. See {ref}`Time-Averaged Statistics <timeAverageStatistics_userGuide>` for how the windows behave.
