...
> - The same selection is available through the Python API ({py:attr}`~flow360.TimeAverageSliceOutput.statistics`). See {ref}`Time-Averaged Statistics <timeAverageStatistics_userGuide>` for how the accumulation windows behave.
...
- **How do I get fluctuation intensity from time-averaged slices?**  
  > RMS is written by [Statistics function](#statistics-function), but it is taken on the raw signal, so it is not a fluctuation intensity about the mean. For that, export both instantaneous and time-averaged slice data and post-process using ParaView/Tecplot.
