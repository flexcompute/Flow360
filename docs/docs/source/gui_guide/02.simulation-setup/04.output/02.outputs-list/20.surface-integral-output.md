...
```{note}
Define the variable as the **local** quantity at a point on the surface. Area weighting is applied for you, so do not multiply by area yourself. The result is reported under the variable's name with an `_integral` suffix.
```
