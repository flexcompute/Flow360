...
```{seealso}
For details on how the reference velocity, Mach/Reynolds numbers, and force/moment coefficients are non-dimensionalized, see {doc}`/user_guide/NonDimensionalization/NonDimensionalization`.
```
...
> - Use the eye control beside this field to show or hide the alpha arc in the viewer (see [Flow direction in the viewer](#flow-direction-in-the-viewer)).
...
>  - Use the eye control beside this field to show or hide the beta arc in the viewer (see [Flow direction in the viewer](#flow-direction-in-the-viewer)).
>  - Affects velocity components according to these formulas:
>    - $U_∞ = U_{mag} · cos(β) · cos(α)$
>    - $V_∞ = -U_{mag} · sin(β)$
>    - $W_∞ = U_{mag} · cos(β) · sin(α)$
...
### Flow direction in the viewer

*An annotation in the 3D viewer showing the freestream direction produced by the current Alpha angle and Beta angle, so the orientation can be checked before running.*

The annotation is made up of an arrow along the freestream direction, dashed projections of that arrow onto the XZ and XY planes, and an arc for each angle drawn between the arrow and its projection. Each arc carries a label naming the angle it measures, and the whole annotation is framed by a box around the model.

Each angle is shown independently. The eye control beside **Alpha angle** toggles the alpha arc and its label, and the one beside **Beta angle** does the same for beta, so a single angle can be inspected without the other on screen.

The annotation follows the values in the panel: change either angle and the arrow and arcs update to match.

### Thermal state

*Choose a method of defining the gas physical properties.*

>**Note:** The gas material is air by default. Its properties (the viscosity model, thermally perfect gas species, and Prandtl numbers) can be edited in the **[Material](../03.flow-solver/03.physics/01.fluid/07.material.md)** section of the Fluid model.
...
- **Can I specify a different material other than Air?**  
  > The GUI provides air for gas conditions and water for liquid conditions. Their properties can be edited in the [Material](../03.flow-solver/03.physics/01.fluid/07.material.md) section of the Fluid model. For materials beyond these, use the Python API. 
...
```{seealso}
Python API:
- {py:class}`~flow360.AerospaceCondition`
- {py:class}`~flow360.ThermalState`
```
