...
*Import a rotor definition from CHARM input files (Python API only).*

- CHARM input is supplied as a file pair for a single rotor: a blade-geometry (`.bg`) file and an airfoil-tables (`.af`) file
- The pair provides blade geometry and sectional polars; the number of blades is not read from the files and must be specified separately
- There is no CHARM control in this panel; the format is reachable only through `BETDisk.from_charm` in the Python API
