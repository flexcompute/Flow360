...
| [**Baffle face treatment**](#baffle-face-treatment) | How baffle faces found during hidden geometry removal are handled | `advanced`, GeometryAI, [**Remove hidden geometry**](#remove-hidden-geometry) is `On` |
...
>  - Two mechanisms share this single value: one caps small planar openings on the imported geometry, the other closes openings while the surface is reconstructed. Which one applies to a given opening depends on the geometry, not on any extra setting. See {ref}`gai_sealing` for what each can and cannot close
...
*Setting controlling how baffle faces are handled. A baffle face is an input face with both sides facing the exterior flow, detected during hidden geometry removal.*

**Possible selections:**
- `Default` - Baffle faces pass through this stage unchanged. No special baffle handling is applied here, but they can still be modified further downstream in the GeometryAI process, for example when **Sealing size** is nonzero and gap sealing closes the gap around a baffle
- `Remove` - Baffle faces are deleted
- `Thicken` - Each baffle is inflated into a thin closed shell so it survives as a manifold part of the mesh

- **Default:** `Default` (no special baffle handling at this stage)
- **Requires:** **Remove hidden geometry** set to `On`. Selecting `Remove` or `Thicken` without it is rejected at validation
- **Advanced option** (found under the *Advanced* section in the UI)

```{figure} ../../../user_guide/Meshing/Figures/gai_baffle_face_treatment.svg
:align: center
:width: 100%

The three selections applied to a baffle face. `Thicken` builds the shell symmetrically about the original face, so the face keeps its in-plane extent and only gains thickness.
```

>**Notes:**
>  - Only available when using GeometryAI
>  - Baffle faces are detected as part of hidden geometry removal
>  - This is a meshing **preprocessing** step: with `Remove` or `Thicken`, a baffle face does not survive as a non-manifold face in the final mesh. It is not the same thing as a *sample surface* (see the note below)
>  - The same choice can be made for individual face groups with **Baffle face treatment** on a [Geometry Refinement](./06.refinements/07.geometry-refinement.md), added under **Refinements**, which overrides this global setting on the faces it lists
...
| --- | --- | --- |
| **Baffle face** | A non-manifold input face (both sides facing the exterior) that the mesher removes or thickens into a manifold mesh, controlled by **Baffle face treatment** above. A meshing preprocessing step (GeometryAI only). | Cleaning up CAD before meshing. The baffle does not survive as a non-manifold face in the final mesh. |
