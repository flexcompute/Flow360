...
- Refinements:
  ...
  - structured box volumetric refinements using boxes
...
.. seealso::
   ...
   Setup: :doc:`WebUI </gui_guide/02.simulation-setup/02.mesh/06.refinements/03.boundary-layer-refinement>` · :class:`Python <flow360.BoundaryLayer>` for the per-surface override, :class:`~flow360.MeshingDefaults` / :class:`~flow360.VolumeMeshingDefaults` for the global first-layer thickness and growth rate
...
- *Structured box refinement*: a semi-structured mesh inside a box-aligned region with independent spacings along the three box axes. Define the box (centre, size, principal axes) and supply the three spacings. The refinement cannot enclose or intersect any other refinement object. It requires the beta mesher: with the legacy mesher the setup is rejected at validation.
...
.. seealso::
   ...
   - Uniform refinement: :doc:`WebUI </gui_guide/02.simulation-setup/02.mesh/06.refinements/05.uniform-refinement>` · :class:`Python <flow360.UniformRefinement>`
   - Axisymmetric refinement: :doc:`WebUI </gui_guide/02.simulation-setup/02.mesh/06.refinements/06.axisymmetric-refinement>` · :class:`Python <flow360.AxisymmetricRefinement>`
   - Structured box refinement: :doc:`WebUI </gui_guide/02.simulation-setup/02.mesh/06.refinements/08.structured-box-refinement>` · :class:`Python <flow360.StructuredBoxRefinement>`

   The region a structured box refinement acts on is a :class:`~flow360.Box` entity (:doc:`WebUI </gui_guide/04.entities-browser/04.volumes/00.box>`): its first and second principal axes are the ones you set on the box, the third is computed, and the three spacings follow that order.
...
.. _custom_volumes_and_zones:

Custom volumes and zones
========================

How many zones the mesher produces
----------------------------------
...
How the farfield boundary is obtained depends on the farfield type: an *automated farfield* is generated automatically around the geometry, sized from its bounding box, whereas a *user-defined farfield* takes its boundaries from the supplied geometry file and meshes the region inside that geometry.

Defining *custom volumes* adds volume zones to that default fluid zone, so internal regions can carry their own mesh. How many zones this comes to depends on how the custom volumes are defined and on the surface mesher in use, which is covered in :ref:`zones_per_definition`. On the solver side, custom zones default to *fluid*; mark a zone as solid to use it in a conjugate heat transfer simulation.

Defining a custom volume
------------------------

Two ways to define the geometry of a custom volume:

- *Enclosing-surface custom volume*: list the surfaces whose closed shell bounds the region. Analytical primitives (cylinders, spheres, axisymmetric bodies) qualify as bounding entities only when they themselves define surfaces, which currently happens only for the interface of a rotation volume. Primitives that define *refinement regions* (axisymmetric or structured-box refinements) are volumetric regions of structured elements, not surfaces, and cannot be used to bound a custom volume.
- *Seedpoint volume*: provide the coordinates of one or more interior seed points. The mesher runs a flood fill from those points to identify the region, which is convenient for cavities bounded by many baffles. Available from release 25.10. Seedpoint volumes require the *GAI surface mesher* or *snappy*; they are not supported with the *beta surface mesher* or the *legacy mesher*.
...
.. _zones_per_definition:

Zones produced by each definition
---------------------------------

- *Enclosing-surface custom volumes*: each enclosing set produces one zone, so several custom volumes produce several zones.
- *Seedpoint volumes with snappy*: each seedpoint volume produces one zone, and each must carry exactly one seed point.
- *Seedpoint volumes with the GAI surface mesher*: the setup produces one zone in total. The simulation may define exactly one *custom zones* container, and that container may hold only seedpoint volumes (it cannot be combined with enclosing-surface custom volumes, nor with a second *custom zones* container). Every seed point of every seedpoint volume in that container is pooled into a single flood fill, so several seed points describe one connected zone (useful when baffles split a region into several compartments) rather than a zone each.

For how seed placement interacts with ``mesh_exterior`` and which combination to use per scenario, see :ref:`gai_best_practices`.

.. seealso::

   Setup: :doc:`WebUI </gui_guide/02.simulation-setup/02.mesh/04.custom-zones>` · :class:`Python <flow360.CustomZones>`, holding :class:`~flow360.CustomVolume` (enclosing surfaces) or :class:`~flow360.SeedpointVolume` (seed points)
...
.. seealso::
   ...
   Setup: :doc:`WebUI </gui_guide/02.simulation-setup/02.mesh/03.rotation-zones>` · :class:`Python <flow360.RotationVolume>` for cylinder and axisymmetric-body interfaces, :class:`~flow360.RotationSphere` for spherical ones
...
.. seealso::
   ...
   Setup: :doc:`WebUI </gui_guide/02.simulation-setup/02.mesh/06.refinements/04.passive-spacing>` · :class:`Python <flow360.PassiveSpacing>`
