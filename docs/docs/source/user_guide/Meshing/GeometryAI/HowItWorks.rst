...
Since GeometryAI reconstructs a watertight surface from the input, it exposes several controls for cleaning up imperfect or overly detailed CAD before meshing. They fall into four groups: sealing openings, preserving thin features, removing geometry hidden from the flow, and handling baffle faces. All of the options below are specific to the GeometryAI Surface Mesher.
...
An opening is closed by offsetting the reconstructed surface, so the geometry around it has to be thick enough to carry that offset. This is why an opening wider than the surrounding geometry is thick cannot be closed this way: the offset that would bridge the gap would also consume the thin wall beside it.

.. figure:: ../Figures/gai_gap_sealing_limit.png
   :align: center
   :width: 100%

   Offset-based sealing closes an opening only where the surrounding geometry is thicker than the gap is wide.

A second, complementary mechanism works on the imported geometry itself, capping small planar openings before the surface is reconstructed. It reaches cases the offset above cannot, in particular an opening wider than the geometry is thick. It introduces no parameter of its own: it uses the same ``Sealing size``, resolved per face group and per :doc:`Geometry Refinement </gui_guide/02.simulation-setup/02.mesh/06.refinements/07.geometry-refinement>` box in the same way as the other cleanup sizes. It is available through the V2 CAD importer.

An opening is capped when it is bounded by a closed loop of edges belonging to a *single face group*, is close to planar, and its smaller in-plane extent is below ``Sealing size``. The face-group condition is the one that most often decides the outcome: where the surrounding faces and the wall of the opening carry separate face ids, the loop closes within one of them and the opening is capped, whereas if they share a face id (or were grouped together upstream) no such loop exists and this step does not see the opening at all. It then falls back to the offset-based sealing above, with the thickness limitation that comes with it.

.. figure:: ../Figures/gai_hole_sealing_faces.png
   :align: center
   :width: 100%

   Geometry-level sealing follows the face layout: separate face ids for the surrounding faces and the wall of the opening give a closed loop that can be capped, while a single shared face id leaves nothing for this step to close.
...
Handling baffle faces
---------------------

``Baffle face treatment`` (:doc:`WebUI </gui_guide/02.simulation-setup/02.mesh/02.mesh-parameters>`, :py:attr:`~flow360.MeshingDefaults.baffle_face_treatment`): decides what happens to the *baffle faces* (faces exposed to the flow on both sides, such as baffles and zero-thickness walls) that are detected as part of hidden geometry removal. ``remove`` deletes them and ``thicken`` inflates each one into a thin closed shell so that it survives as a manifold part of the mesh. ``default``, the default value, applies no special baffle handling at this stage: the baffle faces pass through geometry cleanup unchanged and are left to the rest of the GeometryAI process, which may still modify them (for example, a nonzero ``Sealing size`` seals the gap around a baffle). Any choice other than ``default`` requires ``Remove hidden geometry`` to be on.

.. figure:: ../Figures/gai_baffle_face_treatment.svg
   :align: center
   :width: 100%

   The three treatments applied to a baffle face. ``thicken`` builds the shell symmetrically about the original face, so the face keeps its in-plane extent and only gains thickness.

The same three choices can also be made for an individual face group, through a per-face override set on a :doc:`Geometry Refinement </gui_guide/02.simulation-setup/02.mesh/06.refinements/07.geometry-refinement>`, added under ``Refinements`` in the WebUI (:py:attr:`~flow360.GeometryRefinement.baffle_face_treatment` in the Python API). It overrides the global setting on the faces the refinement lists, while face groups that leave it unset keep the global value. A per-face override applies to surfaces only, so it cannot be combined with a :py:class:`~flow360.Box` region in the same refinement.
...
- ``Geometry Refinement`` (:doc:`WebUI </gui_guide/02.simulation-setup/02.mesh/06.refinements/07.geometry-refinement>`, :py:class:`~flow360.GeometryRefinement`) overrides ``Geometry accuracy`` (and, optionally, ``Preserve thin geometry``, ``Sealing size``, and ``Min passage size``) on a selected set of faces. This is the recommended way to resolve a small or detailed feature without refining the whole model.
  ...
  The same refinement can instead be scoped to a :py:class:`~flow360.Box` region, which suits features known by location rather than by face.
...
For symmetric configurations, the mesher can split the geometry into a half-model about the symmetry plane, which roughly halves the mesh size and is well suited to straight-line (zero-yaw) simulations. It is requested with ``Domain type`` (:doc:`WebUI </gui_guide/02.simulation-setup/02.mesh/01.farfield>`, :py:attr:`~flow360.AutomatedFarfield.domain_type`), which also selects which side of the plane is kept. The half-model option requires the beta mesher, together with the GeometryAI Surface Mesher when starting from a geometry.

Supplying this setting explicitly overrides the automatic symmetry plane detection that otherwise infers the domain from the geometry bounding box, and a half-model whose geometry lies entirely on the discarded side is rejected rather than meshed. The user-defined and wind tunnel farfields take the same setting (:py:attr:`~flow360.UserDefinedFarfield.domain_type`, :py:attr:`~flow360.WindTunnelFarfield.domain_type`), but only together with the GeometryAI Surface Mesher, and therefore only when starting from a geometry. Starting from a surface mesh, a half-model can be requested on the automated farfield alone.
