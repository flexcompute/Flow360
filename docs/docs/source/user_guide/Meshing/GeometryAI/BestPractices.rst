...
One caveat applies when the case has several volume zones: hidden-geometry removal is not fully supported in that combination, and the setup reports an advisory message saying so. The mesh still runs.

The parameter itself, together with ``Min passage size`` (:py:attr:`~flow360.MeshingDefaults.min_passage_size`) and ``Baffle face treatment`` (:py:attr:`~flow360.MeshingDefaults.baffle_face_treatment`), is described under :ref:`gai_geometry_cleanup`.

The setup reports three messages around this toggle:

- With more than one body meshed as exterior and the toggle off, it recommends turning the toggle on; the recommendation is advisory and the mesh still runs.
- ``Min passage size`` and ``Baffle face treatment`` (:py:attr:`~flow360.MeshingDefaults.baffle_face_treatment`) require the toggle: in the meshing defaults they are rejected, in :class:`~flow360.GeometryRefinement` they are accepted but have no effect.
- Hidden-geometry removal is not fully supported when the case generates more than one farfield, custom or seed point volume zone.
...
Defining a ``Seed point`` volume explicitly (:doc:`WebUI </gui_guide/04.entities-browser/04.volumes/05.seed-point-volume>`, :py:attr:`~flow360.SeedpointVolume.point_in_mesh`) is covered on the seed point volume page of the WebUI guide and, on the Python side, in :ref:`new_volume_mesher_user_guide`. Seed points describe a single fluid zone with GeometryAI, however many seed point volumes and points are defined; :ref:`zones_per_definition` covers that and how several zones are obtained instead.
