...
def simplify_signature(sig: str) -> str:
    ...
    sig = _UNYT_SIG_RE.sub(lambda m: f"{m.group(1)} * {_prefix_units(m.group(2))}", sig)
    prev = None
    while sig != prev:
        prev = sig
        sig = _MODEL_SIG_RE.sub(
            lambda m: (
                f"{m.group(1)}()" if "=" in m.group(0)[len(m.group(1)) + 1 : -1] else m.group(0)
            ),
            sig,
        )
    return sig
