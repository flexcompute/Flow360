...
def _import_dimensioned_type():
    ...
    if _DIMENSIONED_TYPE_BASE is None:
        from flow360.component.simulation.unit_system import _DimensionedType
...
def _vector_suffix(cls_obj) -> str:
    ...
    length = _extract_closure_var(getattr(cls_obj, "__get_pydantic_json_schema__", None), "length")
...
def _matrix_suffix(cls_obj) -> str:
    shape = _extract_closure_var(getattr(cls_obj, "__get_pydantic_json_schema__", None), "shape")
    if shape == (None, 3):
