...
:code:`from_c81` and :code:`from_xfoil` additionally require :code:`number_of_blades`, because the C81 and XFOIL inputs carry no blade-count information (XROTOR and DFDC files already include it). For an unsteady :ref:`BET Line <bet_disk_knowledge_base>` simulation, also provide :code:`blade_line_chord` and :code:`initial_blade_direction`.
...
.. seealso::
   ...
   - The :doc:`BETDisk Python API reference </python_api/API_reference/_autosummary/flow360.BETDisk>` for the full parameter list and the :code:`from_xrotor`, :code:`from_dfdc`, :code:`from_c81` and :code:`from_xfoil` constructors.
