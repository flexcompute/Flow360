...
.. note::
    ...
    There is another CAD geometry for the ONERA M6 Wing with slightly different specifications in `the NASA Turbulence Modeling Resource website <https://turbmodels.larc.nasa.gov/onerawingnumerics_val.html>`_.
...
After adding the face and group attribute, which is shown in the above :ref:`example <face_group_attribute>`, since we want to have the same `grid settings <https://turbmodels.larc.nasa.gov/Onerawingnumerics_grids/input.nml_wing_tetra_stt>`_ to compare with `the NASA Turbulence Modeling Resource website <https://turbmodels.larc.nasa.gov/onerawingnumerics_val.html>`_ , we set root chord = 1 by scaling the whole solid body with 1/0.8059 around the origin.
