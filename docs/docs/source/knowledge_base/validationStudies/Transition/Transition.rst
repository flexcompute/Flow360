...
The flat plate case is based on the `model verification case available on the NASA TMR website <https://tmbwg.github.io/turbmodels/flatplate.html>`_. The freestream Mach number is equal to 0.2 with the Reynolds number per unit length equal to 5 million. The boundary conditions applied are based on the Flat Plate TMR Case as shown in :ref:`Fig1_FlatPlate_BoundaryConditions`.
...
The simulations are performed using SA-AFT model with an :math:`N_{crit}` value of 10.3 for consistency with the study of `Coder <https://arc.aiaa.org/doi/10.2514/6.2019-0039>`_ and data from `experiments <https://ntrs.nasa.gov/citations/19930092285>`_ for the skin friction coefficient. The grids were taken from the `NASA TMR Flat Plate grids download page <https://tmbwg.github.io/turbmodels/flatplate_grids.html>`_ with five grid levels available ranging from 35 by 25 to 545 by 385 in the x-z plane with 2 nodes in the spanwise direction, as shown in :ref:`Tab1_GridRef_FlatPlate`.
