...
class Flow360BaseModel(pd.BaseModel):
    ...
    def copy(self, update: dict[str, Any] | None = None, **kwargs: Any) -> Flow360BaseModel:
        ...
        # A copy is a new object: drop the source identity, but honor an id the
        # caller explicitly chose for the copy.
        data = new_copy.model_dump(exclude={"private_attribute_id"})
        if update and "private_attribute_id" in update:
            data["private_attribute_id"] = update["private_attribute_id"]
    ...
    def preprocess(
        ...
        *,
        ...
        Copy-on-write: returns ``self`` when no value in the subtree required
        conversion, skipping the pydantic re-validation rebuild for unchanged nodes.
        """
        if exclude is None:
            exclude = []
        ...
        convertible = convertible_field_names(self.__class__)
        if not convertible:
            return self
        ...
        changed = any(solver_values[name] is not value for name, value in self_values.items())
        for property_name, value in self_values.items():
            if property_name in exclude or property_name not in convertible:
                continue
            ...
            if isinstance(value, Flow360BaseModel):
                new_value = _preprocess_any_model(
                    value,
                    params=params,
                ...
                new_value = _preprocess_nested(value, [*required_by, loc_name], params, exclude, flow360_unit_system)
            else:
                continue
