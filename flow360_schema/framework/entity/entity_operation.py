...
# How many full turns `canonicalize_axis_and_angle` is willing to reduce away. At 1000
# turns the reduction error stays under 1e-12 radians, far below anything a mesher
# resolves; at 1e16 radians it would reach 0.3 in the rotation matrix, which is a
# different box rather than rounding noise.
_MAX_REDUCIBLE_TURNS = 1000.0
...
def canonicalize_axis_and_angle(axis: Axis, angle_in_radians: float) -> tuple[Axis, float]:
    """Collapse the equivalent spellings of one rotation onto a single representative.

    Axis-angle is not a unique encoding: ``(a, theta)``, ``(-a, -theta)`` and
    ``(a, theta + 2*pi*k)`` all describe the same rotation, a half turn leaves the axis
    sign free, and a zero rotation leaves the axis arbitrary. The same box therefore
    reaches serialization spelled differently depending on how it was built -- through
    ``Box.from_principal_axes``, through an explicit axis and angle, or through the field
    defaults -- and two identical setups hash differently, which re-runs meshing that
    could have been reused.

    The representative has ``theta`` in ``[0, pi]``, ``(0, 0, 1)`` for a zero rotation,
    and, for a half turn, the axis whose first non-zero component is positive.

    Comparisons here are exact, so this collapses only the spellings that coincide
    exactly: a sign flip, a whole number of turns, and the free choices at zero and at a
    half turn. Encodings that merely agree to within rounding -- ``0.1`` against
    ``0.1 + 2*pi``, or an eigen-decomposed axis against a hand-written one -- still differ
    in their last bits and still hash apart. Closing that gap needs a quantization tied to
    the mesher's own precision, which is a separate decision from removing exact duplicates.
    """
    full_turn = 2.0 * math.pi
    components: tuple[float, ...] = tuple(axis)
    if abs(angle_in_radians) > _MAX_REDUCIBLE_TURNS * full_turn:
        # Reducing costs roughly |angle| * 1.4e-16 radians, because the double nearest to
        # 2*pi is itself half an ulp off and that error is multiplied by the number of
        # turns removed. Past this many turns the reduction would move the box further
        # than the tolerance below, so the angle is handed back exactly as given: an
        # un-canonicalized rotation only forfeits reuse, a silently rotated one is wrong.
        return Axis(tuple(component + 0.0 for component in components)), angle_in_radians
    angle = angle_in_radians % full_turn
    if angle > math.pi:
        # A turn past pi is the mirror-image turn about the flipped axis.
        angle = full_turn - angle
        components = tuple(-component for component in components)
    if angle == 0.0:
        return Axis((0.0, 0.0, 1.0)), 0.0
    if angle == math.pi:
        leading_sign = next(math.copysign(1.0, component) for component in components if component != 0.0)
        components = tuple(leading_sign * component for component in components)
    # Negating or sign-flipping turns a zero component into -0.0, which serializes
    # differently from 0.0 while meaning the same axis; `+ 0.0` folds the two together.
    return Axis(tuple(component + 0.0 for component in components)), angle
...
class TransformableEntity:
    """Marks an entity that a coordinate system may be assigned to.

    Inheriting this is what makes an entity accepted by
    ``DraftContext.coordinate_systems.assign``, so a new movable entity declares itself
    here rather than in a list kept somewhere else.

    Most subclasses implement ``_apply_transformation`` to return a moved copy of
    themselves, which is what the translator calls when preparing a run. Two entities are
    moved by a consumer instead and so carry the marker without the hook:
    ``GeometryBodyGroup``, whose matrix is handed to the surface mesher, and
    ``ImportedSurface``, whose matrix the solver receives as ``transformationMatrix``.
    Being transformable therefore means "some consumer reads a matrix for this entity",
    not "this class implements the hook".
    """
