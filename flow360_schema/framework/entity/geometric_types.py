...
def _validate_coordinate(value: object) -> tuple[float, float, float]:
    ...
    # Strings must fail with ValueError (not convert) so unions fall through to expression branches.
    if any(isinstance(value[i], str) for i in range(3)):  # type: ignore[index]
        raise ValueError(f"All elements must be numeric, got {value}")
    try:
        return (float(value[0]), float(value[1]), float(value[2]))  # type: ignore[index]
    except (TypeError, ValueError) as exc:
        raise ValueError(f"All elements must be numeric, got {value}") from exc
