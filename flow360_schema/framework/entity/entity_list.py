...
# Asked once per item of every stored_entities list, on lists that reach 140k entities.
_is_entity = subclass_predicate(EntityBase)
...
class EntityList(Flow360BaseModel, metaclass=_EntityListMeta):  # type: ignore[metaclass]
    ...
    @pd.field_serializer("stored_entities", when_used="json")
    def _serialize_stored_entities_as_references(self, value: list[Any], info: pd.SerializationInfo) -> list[Any]:
        """JSON dumps emit compact adjacent-run reference groups.

        Definitions live once in ``asset_cache.project_entity_info``; the wire
        carries only ordered ``{"type": ..., "ids": [...]}`` groups. Python-mode
        dumps keep live instances (lossless internal round trips). Solver-internal
        dumps (translator/mesher payloads) opt out via the
        ``inline_stored_entities`` serialization context flag.
        """
        if isinstance(info.context, dict) and info.context.get("inline_stored_entities"):
            return value
        from flow360_schema.framework.entity.entity_utils import group_stored_entity_references

        return group_stored_entity_references(value)
    ...
    def _filter_stored_entities(cls, value: list[Any]) -> list[Any]:
        ...
        stored_entities = [item for item in value if _is_entity(item)]
    ...
    def _ensure_entities_after_expansion(self, param_info: Any) -> "EntityList":
        ...
        if not resolved_entities:
            from flow360_schema.framework.validation.context import get_validation_levels

            # Emptiness is a red field for leveled (workflow) validation and for live
            # construction — but level-less deserialization (validation_level=None:
            # asset params loading, from_file) must tolerate by-design-incomplete
            # params, e.g. a fresh draft whose selector matches nothing yet.
            if get_validation_levels() != []:
