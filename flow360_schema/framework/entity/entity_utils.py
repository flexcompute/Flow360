...
def is_entity_reference_group(item: Any) -> bool:
    """Check whether an item is a compact entity reference group: {"type": <type_name>, "ids": [...]}."""
    return isinstance(item, dict) and set(item.keys()) == {"type", "ids"}
...
def subclass_predicate(cls: type | tuple[type, ...]) -> Callable[[Any], bool]:
    """Build an ``isinstance(obj, cls)`` that remembers its answer per class.

    Pydantic gives every model class a metaclass ``__instancecheck__``, so an isinstance
    against one runs Python code rather than the interpreter's fast path — measured at
    ~450 ns, against ~40 ns for a dict lookup, and against a tuple of types it pays that
    for every member. The answer depends on nothing but the object's class, and entity
    classes are a small stable set while the entities themselves run into the
    hundred-thousands, so it is worth remembering.

    Only for a target whose isinstance answer the class alone decides: a class or a tuple
    of classes, not an ABC that overrides ``__subclasshook__``.
    """
    verdicts: dict[type, bool] = {}

    def is_instance(obj: Any) -> bool:
        obj_class = type(obj)
        verdict = verdicts.get(obj_class)
        if verdict is None:
            verdict = verdicts[obj_class] = isinstance(obj, cls)
        return verdict

    return is_instance


def group_stored_entity_references(stored_entities: list[Any]) -> list[dict[str, Any]]:
    """Serialize a stored_entities list as ordered adjacent-run reference groups.

    Every entity contributes only its ``(type, id)`` — definitions live once in
    ``private_attribute_asset_cache.project_entity_info``. Assignment order is
    preserved; duplicates are kept verbatim (dedup is upstream responsibility).
    An entity without a type or id cannot be referenced and fails loudly.
    """
    groups: list[dict[str, Any]] = []
    for index, entity in enumerate(stored_entities):
        type_name = getattr(entity, "private_attribute_entity_type_name", None)
        entity_id = getattr(entity, "private_attribute_id", None)
        if not isinstance(type_name, str) or not isinstance(entity_id, str) or not entity_id:
            raise ValueError(
                f"[Serializer] stored_entities[{index}] cannot be compacted into a reference: "
                f"type={type_name!r}, id={entity_id!r} ({entity!r})."
            )
        if groups and groups[-1]["type"] == type_name:
            groups[-1]["ids"].append(entity_id)
        else:
            groups.append({"type": type_name, "ids": [entity_id]})
    return groups
