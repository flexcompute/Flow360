...
class EntityRegistry(Flow360BaseModel):
    ...
    def fast_register(self, entity: EntityBase, known_entity_keys: set[tuple[Any, ...]]) -> set[tuple[Any, ...]]:
        """
        ...
        Parameters:
            ...
            known_entity_keys (set[tuple]): Dedup keys (see EntityBase._dedup_key) of already
                registered entities. Avoids O(N^2) scans when registering a large number of
                entities, without serializing each entity for a content hash.

        Returns:
            known_entity_keys (set[tuple])
        """
        entity_type = type(entity)
        if entity_type not in self.internal_registry:
            self.internal_registry[entity_type] = []

        key = entity._dedup_key()
        if key in known_entity_keys:
            return known_entity_keys
        known_entity_keys.add(key)
    ...
    def contains(self, entity: EntityBase) -> bool:
        ...
        """
        entity_type = type(entity)
        return entity_type in self.internal_registry and entity in self.internal_registry[entity_type]
    ...
    def _register_from_entity_info(self, entity_info: Any) -> None:
            ...
            if group_tag and group_tag in attribute_names:
                ...
                if idx < len(grouped_items):
                    for entity in grouped_items[idx]:
                        nonlocal known_entity_keys
                        known_entity_keys = self.fast_register(entity, known_entity_keys)
                else:
        ...
        if entity_info.type_name == "GeometryEntityInfo":
            ...
            for boundary in entity_info.boundaries:
                known_entity_keys = self.fast_register(boundary, known_entity_keys)
            for zone in entity_info.zones:
                known_entity_keys = self.fast_register(zone, known_entity_keys)
