...
class EntityBase(Flow360BaseModel, metaclass=ABCMeta):
    ...
    def copy(self, update: dict[str, Any], **kwargs: Any) -> "EntityBase":  # type: ignore[override]
        ...
        if "private_attribute_id" not in update and type(self).model_fields["private_attribute_id"].is_required():
            # Types without an id factory get a deterministic identity derived
            # from the copy's unique new name (uuid-factory types mint their own).
            update = {**update, "private_attribute_id": update["name"]}
    ...
    def _dedup_key(self) -> tuple[Any, ...]:
        """Cheap registry dedup key derived from identity fields.

        private_attribute_id is mandatory, so identity fields suffice — no
        content hashing needed.
        """
        return (
            type(self).__name__,
            self.private_attribute_id,
            self.name,
            getattr(self, "private_attribute_tag_key", None),
        )
