...
from .validators import finite_or_raise
...
def _create_validator(
    ...
    def validate(value: Any, info: ValidationInfo) -> Any:
        ...
        # A dimensioned physical quantity is never legitimately NaN/Inf; the only exception is the
        # NaN sentinel used by runtime expressions, which opts out via the allow_inf_nan context flag.
        # Finiteness is orthogonal to ConstraintKind (RANGE/NO_RANGE), so it is enforced unconditionally.
        allow_inf_nan = bool(info.context.get("allow_inf_nan", False)) if info.context else False
        if not allow_inf_nan:
            finite_or_raise(value_for_validation, physical_dimension_meta.name)
