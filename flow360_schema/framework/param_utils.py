...
def register_entity_list(
    ...
    known_entity_keys: set[tuple[Any, ...]] | None = None,
) -> None:
    """Register all entities reachable from a model into a registry."""
    if known_entity_keys is None:
        known_entity_keys = set()

    for field in model.__dict__.values():
        if isinstance(field, EntityBase):
            known_entity_keys = registry.fast_register(field, known_entity_keys)

        if isinstance(field, EntityList):
            for entity in field.stored_entities or []:
                known_entity_keys = registry.fast_register(entity, known_entity_keys)
            continue
        ...
        if isinstance(field, (list, tuple)):
            for item in field:
                ...
                if isinstance(item, Flow360BaseModel):
                    register_entity_list(item, registry, known_entity_keys)
            continue

        if isinstance(field, Flow360BaseModel):
            register_entity_list(field, registry, known_entity_keys)
