"""Whitelisted functions and classes that can be called from expression functions."""
...
class CallableResolver:
    ...
    def __init__(self, callables: Any, modules: Any, imports: Any, blacklist: Any) -> None:
    ...
    def get_allowed_callable(self, qualname: str) -> Callable[..., Any]:
        ...
        try:
            ...
            if qualname in self._callable_builtins or qualname in self._module_builtins:
                ...
                for names, import_func in self._import_builtins.items():
                    if qualname in names or any(qualname.startswith(prefix) for prefix in names):
                        ...
                        return callable_obj  # type: ignore[no-any-return]
