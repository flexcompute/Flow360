...
from flow360_schema.framework.expression.engine.ast_nodes import ExpressionNodeType
...
class AssignNode(StatementNode):
    ...
    def evaluate(
        ...
        context.set_value(self.target, self.value.evaluate(context, raise_on_non_evaluable, force_evaluate))
...
class AugAssignNode(StatementNode):
    ...
    def evaluate(
        ...
        if self.op == "Add":
            context.set_value(self.target, old_val + increment)
        elif self.op == "Sub":
            context.set_value(self.target, old_val - increment)
        elif self.op == "Mult":
            context.set_value(self.target, old_val * increment)
...
class ForLoopNode(StatementNode):
    ...
    def evaluate(
        ...
        for item in iterable:
            context.set_value(self.target, item)
            for stmt in self.body:
                stmt.evaluate(context, raise_on_non_evaluable, force_evaluate)
...
class TupleUnpackNode(StatementNode):
    ...
    def evaluate(
        ...
        evaluated_values = [val.evaluate(context, raise_on_non_evaluable, force_evaluate) for val in self.values]
        # Single expression assigned to multiple targets: unpack the result
        if len(evaluated_values) == 1 and len(self.targets) > 1:
            evaluated_values = list(evaluated_values[0])
        for target, value in zip(self.targets, evaluated_values, strict=True):
