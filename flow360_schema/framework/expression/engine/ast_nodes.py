...
from flow360_schema.framework.expression.engine.types import Evaluable
    ...
    def evaluate(
        ...
        if self.ctx == "Load":
            if isinstance(item, float):
                item = int(item)
            return value[item]  # type: ignore[index]
        if self.ctx == "Store":
...
class RangeCallNode(ExpressionNode):
    ...
    def evaluate(
        ...
        return range(self.arg.evaluate(context, raise_on_non_evaluable, force_evaluate))  # type: ignore[arg-type]
...
class CallModelNode(ExpressionNode):
    ...
    def evaluate(
        ...
        try:
            ...
            # Evaluate arguments
            args = [arg.evaluate(context, raise_on_non_evaluable, force_evaluate) for arg in self.args]
            kwargs = {k: v.evaluate(context, raise_on_non_evaluable, force_evaluate) for k, v in self.kwargs.items()}

            return func(*args, **kwargs)

        except AttributeError as e:
            raise ValueError(f"Invalid attribute in call chain '{self.func_qualname}': {str(e)}") from e
...
class TupleNode(ExpressionNode):
    ...
    def evaluate(
        ...
        return tuple(elem.evaluate(context, raise_on_non_evaluable, force_evaluate) for elem in self.elements)

    def used_names(self) -> set[str]:
        names: set[str] = set()
        for elem in self.elements:
            names = names.union(elem.used_names())
        return names
...
class ListNode(ExpressionNode):
    ...
    def evaluate(
        ...
        return [elem.evaluate(context, raise_on_non_evaluable, force_evaluate) for elem in self.elements]
...
class ListCompNode(ExpressionNode):
    ...
    def evaluate(
        ...
        for item in iterable:
            ...
            iter_context.set_value(self.target, item)
