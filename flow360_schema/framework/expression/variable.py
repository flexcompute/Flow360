...
_ENTITY_INFO_PATH = ("private_attribute_asset_cache", "project_entity_info")

# Subtrees the expression scan never descends into, by exact path from the params root.
# The entity_info containers catalog every face/edge/body/zone of the asset: ids and
# persistent entities only, never expressions, yet on large geometries they hold well
# over 99% of the nodes in the params tree. ``draft_entities``/``ghost_entities`` are
# deliberately absent — user-authored entities can carry expression-valued fields.
_EXPRESSION_SCAN_EXCLUDED_PATHS = frozenset(
    {
        ("private_attribute_asset_cache", "variable_context"),
        *(
            _ENTITY_INFO_PATH + (field_name,)
            for field_name in (
...
def get_referenced_expressions_and_user_variables(param_as_dict: dict[str, Any]) -> list[str]:
    ...
    def _collect_expressions_recursive(
        ...
        if current_path in _EXPRESSION_SCAN_EXCLUDED_PATHS:
            return
    ...
    _collect_expressions_recursive(param_as_dict, used_expressions, current_path=())
