...
class MultiConstructorBaseModel(Flow360BaseModel, metaclass=abc.ABCMeta):
    ...
    type_name: Literal["MultiConstructorBaseModel"] = pd.Field("MultiConstructorBaseModel", frozen=True)
    private_attribute_constructor: str = pd.Field("default", frozen=True)
    private_attribute_input_cache: Any | None = pd.Field(None, frozen=True)

    @classmethod
    def model_constructor(cls, func: Callable[..., Any]) -> Callable[..., Any]:
        """
        ...
        @wraps(func)
        def wrapper(cls: type, **kwargs: Any) -> MultiConstructorBaseModel:
            obj = func(cls, **kwargs)
            ...
            obj._force_set_attr(
                ...
                obj.private_attribute_input_cache.__class__(
            ...
            obj._force_set_attr("private_attribute_constructor", func.__name__)
...
def get_class_method(cls: type, method_name: str) -> Callable[..., Any]:
    """
...
def get_class_by_name(class_name: str, global_vars: dict[str, Any]) -> type:
    """
...
def _add_parent_location_to_validation_error(
    validation_error: pd.ValidationError, parent_loc: str | int | None = None
) -> pd.ValidationError:
    ...
    return pd.ValidationError.from_exception_data(
        ...
        line_errors=updated_errors,  # type: ignore[arg-type]
    )


def model_custom_constructor_parser(model_as_dict: dict[str, Any], global_vars: dict[str, Any]) -> dict[str, Any]:
    """Parse the dictionary, construct the object and return obj dict."""
    constructor_name = model_as_dict.get("private_attribute_constructor")
    if constructor_name is not None and constructor_name != "default":

        def is_optional_argument(annotation: Any) -> bool:
        ...
        model_cls = get_class_by_name(model_as_dict.get("type_name"), global_vars)  # type: ignore[arg-type]
        ...
        for arg_name, arg_obj in constructor_args.items():
            if arg_name in input_kwargs:  # type: ignore[operator]
                input_kwargs_filtered[arg_name] = input_kwargs[arg_name]  # type: ignore[index]
            elif is_optional_argument(arg_obj.annotation) and arg_obj.default is inspect.Parameter.empty:
                input_kwargs_filtered[arg_name] = None
        try:
            with DeserializationContext():
                # The inline flag keeps EntityList dumps as full entity dicts: this
                # re-dump happens AFTER materialization in validate_model's
                # preprocessing, so compact reference groups emitted here would
                # never be resolved. Inline dicts parse directly at model_validate.
            ...
            return model_dict  # type: ignore[no-any-return]
...
def parse_model_dict(model_as_dict: Any, global_vars: dict[str, Any], parent_loc: str | int | None = None) -> Any:
    ...
    try:
        if isinstance(model_as_dict, dict):
            ...
            model_as_dict = [parse_model_dict(item, global_vars, idx) for idx, item in enumerate(model_as_dict)]
    except pd.ValidationError as err:
        raise _add_parent_location_to_validation_error(validation_error=err, parent_loc=parent_loc) from err
