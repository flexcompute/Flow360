...
class RampCFL(Flow360BaseModel):
    ...
    initial: pd.PositiveFloat | None = pd.Field(
        None,
    ...
    final: pd.PositiveFloat | None = pd.Field(
        None,
...
class AdaptiveCFL(Flow360BaseModel):
    ...
    max: pd.PositiveFloat | None = pd.Field(
        None,
    ...
    max_relative_change: pd.PositiveFloat | None = pd.Field(
        None,
        ...
        + "In steady simulations default value is 1. In unsteady simulations default value is 50.",
    )
    convergence_limiting_factor: pd.PositiveFloat | None = pd.Field(
        None,
        ...
        + "In steady simulations default value is 0.25. In unsteady simulations default value is 1.",
    )
    ...
    def default_unsteady(cls):
        ...
        """
        return cls(max=1e6, max_relative_change=50, convergence_limiting_factor=1.0)
    ...
    def default_steady(cls):
        ...
        """
        return cls(max=1e4, max_relative_change=1, convergence_limiting_factor=0.25)
...
class Steady(Flow360BaseModel):
    ...
    CFL: RampCFL | AdaptiveCFL = pd.Field(default=AdaptiveCFL.default_steady(), description="CFL settings.")
...
class Unsteady(Flow360BaseModel):
    ...
    max_pseudo_steps: int = pd.Field(20, gt=0, le=100000, description="Maximum pseudo steps within one physical step.")
    ...
    CFL: RampCFL | AdaptiveCFL = pd.Field(
