"""Minimal conversion helpers used by migrated simulation model modules."""
...
class RestrictedUnitSystem(u.UnitSystem):
    ...
    def __getitem__(self, key):
        ...
        if self._supported_dims is not None:
            ...
            if unsupported:
                names = ", ".join(str(symbol) for symbol in unsupported)
                raise ValueError(
                    f"Cannot non-dimensionalize {key}: " f"base units for {names} are not defined in this context."
                )
...
def compute_udf_dimensionalization_factor(params, requested_unit, using_liquid_op):
    ...
    flow360_unit_system = (
        params.flow360_unit_system if not using_liquid_op else get_flow360_unit_system_liquid(params=params)
    )
    ...
    coefficient, offset = _compute_coefficient_and_offset(source_unit=requested_unit, target_unit=flow360_unit)
    return coefficient, offset
