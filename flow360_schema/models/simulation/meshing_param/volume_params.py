...
from flow360_schema.models.entities.output_entities import Slice
...
def _serialize_segment_spacing(d):
    """Serialize ``dict[AxisymmetricSegment, Length.PositiveFloat64]`` as a list of {segment, spacing} pairs."""
    if d is None:
        return None
    adapter = pd.TypeAdapter(Length.PositiveFloat64)
    return [{"segment": seg.model_dump(), "spacing": adapter.dump_python(spacing)} for seg, spacing in d.items()]


def _deserialize_segment_spacing(v):
    """Deserialize list of {segment, spacing} pairs back into ``dict[AxisymmetricSegment, Length.PositiveFloat64]``."""
...
SegmentSpacingDict = Annotated[
    dict[AxisymmetricSegment, Length.PositiveFloat64],
    pd.BeforeValidator(_deserialize_segment_spacing),
    pd.PlainSerializer(_serialize_segment_spacing),
]
...
class UniformRefinement(Flow360BaseModel):
    ...
    name: str | None = pd.Field("Uniform refinement")
    ...
    def check_entities_used_with_beta_mesher(cls, values, param_info: ParamsValidationInfo):
        ...
        for entity in expanded:
            ...
            if isinstance(entity, Sphere):
                raise ValueError("`Sphere` entity for `UniformRefinement` is supported only with beta mesher.")

        return values

    @contextual_model_validator(mode="after")
    def check_annular_cylinder_used_with_beta_mesher(self, param_info: ParamsValidationInfo):
        """Reject a hollow cylinder the legacy volume mesher cannot express.

        The legacy mesher builds the refinement region from the outer radius alone and its input
        schema rejects the key outright, so such a run fails in its first validation step; caught
        here, at submit, instead.

        A refinement that snappy will project to the surface is left to the snappy validator, whose
        message names snappy. A volume-only refinement (project_to_surface=False) is not snappy's to
        check, so it is not exempt. That is why this is a model validator: project_to_surface is
        declared after entities and a field validator on entities could not see it.
        """
        if param_info.is_beta_mesher:
            return self
        if param_info.use_snappy and self.project_to_surface is not False:
            return self

        for entity in param_info.expand_entity_list(self.entities):
            if isinstance(entity, Cylinder) and entity.inner_radius is not None and entity.inner_radius > 0 * u.m:
                raise ValueError(
                    "Annular `Cylinder` entity (inner_radius > 0) for `UniformRefinement` "
                    "is supported only with beta mesher."
                )
        return self
    ...
    def check_face_spacing(self, param_info: ParamsValidationInfo):
        ...
        for seg in self.face_spacing:
            entity = registry.find_by_type_name_and_id(
                ...
                entity_id=seg.owning_entity_id,
            )
            if entity is None:
                raise ValueError("face_spacing references unregistered AxisymmetricBody.")
            if seg.owning_entity_id not in entity_ids_in_refinement:
                add_validation_warning(
                    f"face_spacing references entity '{entity.name}', which is not in "
...
class StructuredBoxRefinement(Flow360BaseModel):
    ...
    - The :class:`StructuredBoxRefinement` is only supported with the beta mesher.
    ...
    ...         )
    ...     ],
    ...     spacing_axis1=7.5 * fl.u.cm,
    ...     spacing_axis2=10 * fl.u.cm,
    ...     spacing_normal=15 * fl.u.cm,
    ... )

    ====
    """

    name: str | None = pd.Field("StructuredBoxRefinement")
    refinement_type: Literal["StructuredBoxRefinement"] = pd.Field("StructuredBoxRefinement", frozen=True)
    entities: EntityList[Box] = pd.Field()

    spacing_axis1: Length.PositiveFloat64 = pd.Field(
        description="Spacing along the first principal axis of the box, the direction of its first size component."
    )
    spacing_axis2: Length.PositiveFloat64 = pd.Field(
        description="Spacing along the second principal axis of the box, the direction of its second size component."
    )
    spacing_normal: Length.PositiveFloat64 = pd.Field(
        description=(
            "Spacing along the third axis of the box, the one computed automatically to complete the right-handed "
            "coordinate system, and the direction of its third size component."
        )
    )
...
class AxisymmetricRefinement(Flow360BaseModel):
    ...
    name: str | None = pd.Field("Axisymmetric refinement")
    refinement_type: Literal["AxisymmetricRefinement"] = pd.Field("AxisymmetricRefinement", frozen=True)
    entities: EntityList[Cylinder] = pd.Field()
    spacing_axial: Length.PositiveFloat64 = pd.Field(description="Spacing along the axial direction.")
    spacing_radial: Length.PositiveFloat64 = pd.Field(description="Spacing along the radial direction.")
...
class _RotationVolumeBase(Flow360BaseModel):
    ...
    name: str | None = pd.Field(None, description="Name to display in the GUI.")
    enclosed_entities: EntityList[Cylinder, Surface, MirroredSurface, AxisymmetricBody, Box, Sphere] | None = pd.Field(
        None,
        description=(
    ...
    stationary_enclosed_entities: EntityList[Surface, MirroredSurface] | None = pd.Field(
        None,
        description=(
    ...
    def _prevent_direct_instantiation(cls, data):
        if cls is _RotationVolumeBase:
            raise TypeError("`_RotationVolumeBase` is an abstract base model and cannot be instantiated directly.")
        return data
    ...
    def _validate_single_instance_in_entity_list(cls, values, param_info: ParamsValidationInfo):
        ...
        if len(expanded_entities) > 1:
            raise ValueError(f"Only single instance is allowed in entities for each `{cls.__name__}`.")
        return values
    ...
    @classmethod
    def _validate_stationary_enclosed_entities_only_in_beta_mesher(cls, values, param_info: ParamsValidationInfo):
        """
    ...
    def _validate_stationary_enclosed_entities_subset(self, param_info: ParamsValidationInfo):
        ...
        if self.enclosed_entities is None:
            raise ValueError("`stationary_enclosed_entities` cannot be specified when `enclosed_entities` is None.")
        ...
        expanded_stationary_enclosed_entities = param_info.expand_entity_list(self.stationary_enclosed_entities)
...
class RotationVolume(_RotationVolumeBase):
    ...
    name: str | None = pd.Field("Rotation Volume", description="Name to display in the GUI.")
...
class RotationSphere(_RotationVolumeBase):
    ...
    name: str | None = pd.Field("Rotation Sphere", description="Name to display in the GUI.")
    ...
    def _validate_entities_beta_mesher_only(cls, values, param_info: ParamsValidationInfo):
        ...
        if expanded_entities:
            raise ValueError("`Sphere` entity for `RotationSphere` is only supported with the beta mesher.")
        return values
...
class _FarfieldBase(Flow360BaseModel):
    ...
    domain_type: Literal["half_body_positive_y", "half_body_negative_y", "full_body"] | None = (
        pd.Field(  # In the future, we will support more flexible half model types and full model via Union.
            None,
            description="""
            - half_body_positive_y: Trim to a half-model by slicing with the global Y=0 plane;
              keep the '+y' side for meshing and simulation.
            - half_body_negative_y: Trim to a half-model by slicing with the global Y=0 plane;
              keep the '-y' side for meshing and simulation.
            - full_body: Keep the full body for meshing and simulation without attempting
              to add symmetry planes.

            Surfaces lying entirely on the discarded side are dropped from the mesh, and a
            half-model whose geometry lies entirely on that side is rejected.

            Requires the beta mesher, plus the GAI surface mesher unless starting
            from a surface mesh with an automated farfield.

            Warning: When using AutomatedFarfield or UserDefinedFarfield, setting
    ...
    def _validate_only_in_beta_mesher(cls, value, param_info: ParamsValidationInfo):
        """
        Ensure that domain_type is only used with the beta mesher, and with GAI when starting
        from geometry.
        """
        if not value or param_info.supports_farfield_domain_type():
            return value
        if param_info.is_beta_mesher is not True:
            raise ValueError("`domain_type` requires the beta volume mesher.")
        if param_info.root_asset_type == "surface_mesh":
            raise ValueError(
                "Specifying `domain_type` while starting from a surface mesh is only supported on `AutomatedFarfield`; "
                "the user-defined and wind-tunnel farfields use a symmetry wall you supply."
            )
        raise ValueError("`domain_type` requires the GAI surface mesher.")
...
class _FarfieldAllowingEnclosedEntities(_FarfieldBase):
    ...
    enclosed_entities: EntityList[Surface, Cylinder, AxisymmetricBody, Sphere, CustomVolume] | None = pd.Field(
        None,
        description="""
        The surfaces/surface groups that are the interior boundaries of the `farfield`
        zone when defining custom volumes.
...
class AutomatedFarfield(_FarfieldAllowingEnclosedEntities):
    ...
    name: str | None = pd.Field("Automated Farfield")  # Kept optional for backward compatibility
    ...
    def farfield(self):
        ...
        A partial token: geometry fields stay None (the pipeline owns them via
        entity_info.ghost_entities); name/id match what the pipeline generates.
        """
        return GhostSphere(name="farfield", private_attribute_id="farfield")

    @property
    def symmetry_plane(self) -> GhostCircularPlane:
        """
        Returns the symmetry plane boundary surface.
        """
        if self.method == "auto":
            return GhostCircularPlane(name="symmetric", private_attribute_id="symmetric")
    ...
    def symmetry_planes(self):
        ...
        if self.method in ("quasi-3d", "quasi-3d-periodic"):
            return [
                GhostCircularPlane(name="symmetric-1", private_attribute_id="symmetric-1"),
                GhostCircularPlane(name="symmetric-2", private_attribute_id="symmetric-2"),
            ]
        raise Flow360ValueError(f"Unsupported method: {self.method}")

    @contextual_field_validator("method", mode="after")
    @classmethod
    def _validate_quasi_3d_periodic_only_in_legacy_mesher(cls, values, param_info: ParamsValidationInfo):
        """
...
class StaticFloor(Flow360BaseModel):
    ...
    friction_patch_width: Length.PositiveFloat64 = pd.Field(default=2 * u.m, description="Width of friction patch.")
...
class CentralBelt(Flow360BaseModel):
    ...
    type_name: Literal["CentralBelt"] = pd.Field("CentralBelt", description="Floor with central belt.", frozen=True)
    ...
    central_belt_width: Length.PositiveFloat64 = pd.Field(default=1.2 * u.m, description="Width of central belt.")
...
class WindTunnelFarfield(_FarfieldAllowingEnclosedEntities):
    ...
    **Important:** By default, the volume mesher will grow boundary layers on :class:`~flow360.WindTunnelFarfield`.
    ...
    # Tunnel parameters
    width: Length.PositiveFloat64 = pd.Field(default=10 * u.m, description="Width of the wind tunnel.")
    height: Length.PositiveFloat64 = pd.Field(default=6 * u.m, description="Height of the wind tunnel.")
    inlet_x_position: Length.Float64 = pd.Field(default=-20 * u.m, description="X-position of the inlet.")
    outlet_x_position: Length.Float64 = pd.Field(default=40 * u.m, description="X-position of the outlet.")
    floor_z_position: Length.Float64 = pd.Field(default=0 * u.m, description="Z-position of the floor.")

    floor_type: StaticFloor | FullyMovingFloor | CentralBelt | WheelBelts = pd.Field(
    ...
    def right(cls):
        ...
        return WindTunnelGhostSurface(name="windTunnelRight", private_attribute_id="windTunnelRight")

    @classproperty
    def inlet(cls):
        """Return the ghost surface corresponding to the wind tunnel inlet."""
        return WindTunnelGhostSurface(name="windTunnelInlet", private_attribute_id="windTunnelInlet")

    @classproperty
    def outlet(cls):
        """Return the ghost surface corresponding to the wind tunnel outlet."""
        return WindTunnelGhostSurface(name="windTunnelOutlet", private_attribute_id="windTunnelOutlet")

    @classproperty
    def ceiling(cls):
        """Return the ghost surface for the tunnel ceiling."""
        return WindTunnelGhostSurface(name="windTunnelCeiling", private_attribute_id="windTunnelCeiling")

    @classproperty
    def floor(cls):
        """Return the ghost surface for the tunnel floor."""
        return WindTunnelGhostSurface(name="windTunnelFloor", private_attribute_id="windTunnelFloor")
    ...
    def _get_valid_ghost_surfaces(
        floor_string: str | None = "all", domain_string: str | None = None
...
class MeshSliceOutput(Flow360BaseModel):
    ...
    cutoff_radius: Length.PositiveFloat64 | None = pd.Field(
        default=None,
