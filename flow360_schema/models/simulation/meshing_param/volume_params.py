...
class StructuredBoxRefinement(Flow360BaseModel):
    ...
    spacing_axis1: Length.PositiveFloat64 = pd.Field(description="Spacing along the first axial direction.")
    spacing_axis2: Length.PositiveFloat64 = pd.Field(description="Spacing along the second axial direction.")
    spacing_normal: Length.PositiveFloat64 = pd.Field(description="Spacing along the normal axial direction.")
...
class _FarfieldBase(Flow360BaseModel):
    ...
    def _validate_only_in_beta_mesher(cls, value, param_info: ParamsValidationInfo):
        ...
        if not value or (param_info.use_geometry_AI is True and param_info.is_beta_mesher is True):
            return value
        raise ValueError("`domain_type` is only supported when using both GAI surface mesher and beta volume mesher.")
