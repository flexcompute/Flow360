...
class QualityMetrics(Flow360BaseModel):
    ...
    max_non_orthogonality: Angle.PositiveFloat64 | Literal[False] = pd.Field(
        default=85 * u.deg,
        alias="max_non_ortho",
    ...
    max_boundary_skewness: Angle.PositiveFloat64 | Literal[False] = pd.Field(
        default=20 * u.deg,
        description="Maximum boundary skewness. Set to False to disable this metric.",
    )
    max_internal_skewness: Angle.PositiveFloat64 | Literal[False] = pd.Field(
        default=50 * u.deg,
        description="Maximum internal face skewness. Set to False to disable this metric.",
    )
    max_concavity: Angle.PositiveFloat64 | Literal[False] = pd.Field(
        default=50 * u.deg,
        alias="max_concave",
        description="Maximum cell concavity. Set to False to disable this metric.",
    )
    min_pyramid_cell_volume: float | Literal[False] | None = pd.Field(
        default=None,
        alias="min_vol",
    ...
    zmetric_threshold: pd.NonNegativeFloat | Literal[False] = pd.Field(
    ...
    feature_edge_deduplication_tolerance: pd.NonNegativeFloat | Literal[False] = pd.Field(
        default=0.2,
        description=(
    ...
    min_volume_collapse_ratio: float | Literal[False] = pd.Field(
        default=0,
