...
class SurfaceMeshingParams(Flow360BaseModel):
    ...
    type_name: Literal["SnappySurfaceMeshingParams"] = pd.Field("SnappySurfaceMeshingParams", frozen=True)
    ...
    smooth_controls: SmoothControls | Literal[False] = pd.Field(SmoothControls())
    refinements: list[SnappySurfaceRefinementTypes] | None = pd.Field(None)
    octree_spacing: OctreeSpacing | None = pd.Field(None, validation_alias="base_spacing")
    ...
    def _check_body_refinements_w_defaults(self):
        ...
        for refinement in self.refinements:
            if not isinstance(refinement, BodyRefinement):
                continue
            if (
                refinement.min_spacing is None
                and refinement.proximity_spacing is not None
                and refinement.proximity_spacing > self.defaults.min_spacing
            ):
                raise ValueError(
