...
class OctreeSpacing(Flow360BaseModel):
    ...
    def _reject_plain_value(cls, input_data):
        if isinstance(input_data, u.unyt_quantity):
            raise ValueError(
...
class MeshingDefaults(Flow360BaseModel):
    ...
    surface_edge_growth_rate: float = ContextField(
        1.2,
        ge=1,
        description="Growth rate of the anisotropic layers grown from the edges.This can not be overridden per edge.",
    ...
    surface_max_edge_length: Length.PositiveFloat64 | None = ConditionalField(
        None,
    ...
    target_surface_node_count: pd.PositiveInt | None = ContextField(
        None,
    ...
    baffle_face_treatment: Literal["remove", "thicken", "default"] = pd.Field(
        "default",
        description="Treatment of baffle faces (faces with both sides facing fluid) detected during hidden"
        + "geometry removal: 'remove' deletes them, while 'thicken' inflates them into a thin closed shell."
        + "'default' applies no special baffle handling. This only takes effect when hidden "
        + "geometry removal is performed, and is only supported when using geometryAI. ",
    )
    ...
    octree_spacing: OctreeSpacing | None = pd.Field(
        None,
    ...
    def _expand_legacy_remove_baffle_faces(cls, data):
        """Rewrite the legacy boolean ``remove_baffle_faces`` into ``baffle_face_treatment``
        for direct Python construction (JSON handled by the 25.11.3 updater). Enforces
        mutual exclusion with ``baffle_face_treatment``.
        Only maps when remove_hidden_geometry is True; falls back to "default" otherwise.
        """
        if not isinstance(data, dict) or "remove_baffle_faces" not in data:
            return data

        if data.get("baffle_face_treatment") is not None:
            raise ValueError(
                "MeshingDefaults: provide either `baffle_face_treatment` or `remove_baffle_faces`, not both."
            )

        remove = data.pop("remove_baffle_faces")
        if data.get("remove_hidden_geometry") is True:
            # Non-bool values pass through and get rejected downstream
            data["baffle_face_treatment"] = "remove" if remove is True else "thicken" if remove is False else remove

        add_validation_warning(
            "MeshingDefaults: `remove_baffle_faces` is deprecated; pass "
            "`baffle_face_treatment='remove'|'thicken'|'default'` instead."
        )
        return data
    ...
    @pd.model_validator(mode="after")
    def validate_baffle_face_treatment_requires_remove_hidden_geometry(self):
        """Ensure a non-default baffle_face_treatment is only specified when remove_hidden_geometry
        is True, since baffle classification only runs as part of hidden geometry removal."""
        if self.baffle_face_treatment != "default" and not self.remove_hidden_geometry:
            raise ValueError("'baffle_face_treatment' can only be specified when 'remove_hidden_geometry' is True.")
        return self
