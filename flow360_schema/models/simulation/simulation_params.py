...
from flow360_schema.models.simulation.validation.validation_output import (
    ...
    _check_consistent_statistics_across_instances,
    _check_local_cfl_output,
    _check_rms_output_field_name_collision,
    _check_statistics_on_imported_surfaces,
    _check_time_derivative_output,
...
from flow360_schema.models.simulation.validation.validation_simulation_params import (
    ...
    _check_species_transport_not_with_isentropic_solver,
    _check_steady_output_write_count,
...
class _ParamModelBase(Flow360BaseModel):
    ...
    def from_file(cls, filename: str):
        """Load simulation params through ``validate_model``, the official deserializer.

        Sanitizing, version updating, and compact entity-reference materialization all
        happen inside ``validate_model``. For the lenient loader (defaults ingestion
        before entity info exists) use ``load_lenient`` instead.
        """
        ...
        if errors:
            raise Flow360ValueError(f"Loading {filename} failed with validation errors: {errors}")
        return params

    @classmethod
    def load_lenient(cls, params_as_dict: dict) -> _ParamModelBase:
        """Load params from a dict, skipping validation levels and reference materialization.

        This is the LENIENT loader: compact entity references stay unresolved (ghost
        references still load — they are self-describing) and no validation level runs.
        Sanctioned users are the root-item ingestion pipelines that consume the
        by-design-incomplete default simulation.json before entity info exists, and the
        read-only summary CLI. Every other consumer must load through ``validate_model``
        (or ``from_file``).
        """
        model_dict = cls._handle_dict(**params_as_dict)
    ...
    @classmethod
    def _resolve_unit_system_for_input(cls, data: Any, handler: Any) -> Any:
        """Stamp ``unit_system`` from the ambient context, or trust the one already there.

        Serialized input carries its own ``unit_system`` and is validated as
        deserialization, so bare numbers read as SI. Python construction instead takes the
        system from ``with SI_unit_system:`` and rejects an explicit one that disagrees.

        This is a validator rather than an ``__init__`` override on purpose: a custom
        ``__init__`` puts pydantic on its custom-init path, where every model-level
        ``after`` validator runs a second time on the instance it just built.
        """
        if not isinstance(data, dict):
            return handler(data)

        if is_deserializing():
            # Already reading serialized data; the input carries its own unit system and
            # an ambient context, if any, must not override it.
            return handler(data)

        if unit_system_manager.current is None and "unit_system" in data:
            with DeserializationContext():
                return handler(data)

        # Stamps the ambient system, rejecting an explicit one that disagrees. Raises the
        # actionable error when there is no context to take it from.
        _, kwargs = cls._init_check_unit_system(**data)
        return handler({**kwargs, "unit_system": UnitSystemConfig(name=unit_system_manager.current.name)})
...
class SimulationParams(_ParamModelBase):
    ...
    @pd.model_validator(mode="after")
    def check_rms_output_field_name_collision(self):
        """Reject output fields ending in _rms when rms statistics are requested"""
        return _check_rms_output_field_name_collision(self)

    @pd.model_validator(mode="after")
    def check_statistics_on_imported_surfaces(self):
        """Reject non-default statistics on imported surfaces (mean-only averaging path)"""
        return _check_statistics_on_imported_surfaces(self)

    @pd.model_validator(mode="after")
    def check_consistent_statistics_across_instances(self):
        """Require identical statistics across instances of the same time-average output type"""
        return _check_consistent_statistics_across_instances(self)

    @pd.model_validator(mode="after")
    def check_time_derivative_output(self):
        """Only allow time-derivative output fields (e.g. pressureTimeDerivative) when using unsteady simulation"""
        return _check_time_derivative_output(self)
    ...
    @pd.model_validator(mode="after")
    def check_species_transport_not_with_isentropic_solver(self):
        """Multi-species transport is not supported with the CompressibleIsentropic (4x4) solver."""
        return _check_species_transport_not_with_isentropic_solver(self)
    ...
    @pd.model_validator(mode="after")
    def check_steady_output_write_count(params):
        """Warn when a steady output frequency would produce a large number of output files"""
        return _check_steady_output_write_count(params)
    ...
    @property
    def reference_area(self) -> Area.PositiveFloat64:
        """
        This function returns the **reference area**, resolving the recipe and
        expression forms of ``reference_geometry.area`` to a value.
        No default is substituted: params carrying no reference area, such as a
        meshing-only setup, raise instead of reporting a fabricated value.
        """
        area = getattr(self.reference_geometry, "area", None)
        if isinstance(area, ProjectedArea):
            area = area.computed
        if isinstance(area, Expression):
            area = area.evaluate(raise_on_non_evaluable=True, force_evaluate=True)
        if area is None:
            raise Flow360ValueError(
                "Reference area is not available: `reference_geometry.area` is unset"
                " or its projected-area recipe has not been computed."
            )
        return area
    ...
    def has_monitors(self):
        ...
        if self.outputs is None:
            return False
        return any(isinstance(item, (*ProbeOutputTypes, SurfaceIntegralOutput)) for item in self.outputs)
