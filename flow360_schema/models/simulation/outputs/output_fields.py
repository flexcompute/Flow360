...
from flow360_schema.models.simulation.conversion import compute_udf_dimensionalization_factor
...
def _apply_vector_conversion(*, base_udf_expression: str, base_field: str, field_name: str, conversion_factor: float):
...
def _apply_scalar_conversion(*, base_udf_expression: str, base_field: str, field_name: str, conversion_factor: float):
    """Apply conversion for scalar fields"""
    factor = 1.0 / conversion_factor
    return f"double {base_field};" f"{base_udf_expression}" f"{field_name} = {base_field} * {factor};"


def _using_liquid_operating_condition(params) -> bool:
    operating_condition = getattr(params, "operating_condition", None)
    type_name = getattr(operating_condition, "type_name", None)
    return type_name == "LiquidOperatingCondition" or type(operating_condition).__name__ == ("LiquidOperatingCondition")
...
def generate_predefined_udf(field_name, params):
    ...
    coefficient, _ = compute_udf_dimensionalization_factor(
        params=params,
        requested_unit=unit,
        using_liquid_op=_using_liquid_operating_condition(params),
    )
