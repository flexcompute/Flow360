...
from flow360_schema.framework.param_utils import serialize_model_obj_to_id
...
ForceOutputModelType = Annotated[
    Wall | BETDisk | ActuatorDisk | PorousMedium,
...
class ForceOutput(_OutputBase):
    ...
    models: list[ForceOutputModelType | str] = pd.Field(
    ...
    def _convert_model_obj_to_id(cls, value):
        ...
        model_ids = set()
        for model in value:
            model_id = model if isinstance(model, str) else serialize_model_obj_to_id(model_obj=model)
...
class AeroAcousticOutput(Flow360BaseModel):
    ...
    permeable_surfaces: (
        EntityList[Surface, GhostSurface, GhostCircularPlane, GhostSphere, WindTunnelGhostSurface] | None
    ) = pd.Field(None, description="List of permeable surfaces. Left empty if `patch_type` is solid")
