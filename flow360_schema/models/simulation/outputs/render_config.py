...
class PBRMaterial(MaterialBase):
    ...
    def metal(cls, shine=0.5, opacity=1.0):
        ...
        """
        return PBRMaterial(color=(255, 255, 255), opacity=opacity, roughness=1 - shine, f0=(0.56, 0.56, 0.56))
...
class FieldMaterial(MaterialBase):
    """
    :class:`FieldMaterial` maps field values to colors for flow visualization, optionally with a
    Line Integral Convolution (LIC) streamline overlay (see :py:attr:`lic`).

    Example
    -------
    >>> FieldMaterial.rainbow(field="pressure", min_value=0, max_value=100000)
    >>> FieldMaterial.airflow(field="velocity", min_value=0, max_value=100, lic=True)
    """
    ...
    lic: bool = pd.Field(
        default=False,
        description="Enable a Line Integral Convolution (LIC) streamline overlay "
        "(https://en.wikipedia.org/wiki/Line_integral_convolution). Requires a vector output_field: "
        "the streaks follow its projected direction while the surface is still colored by its "
        "magnitude.",
    )
    ...
    @classmethod
    def rainbow(cls, field, min_value, max_value, opacity=1, lic=False):
        """
        ...
        return FieldMaterial(
            opacity=opacity, output_field=field, min=min_value, max=max_value, colormap=colormap, lic=lic
        )

    @classmethod
    def orizon(cls, field, min_value, max_value, opacity=1, lic=False):
        """
    ...
    @classmethod
    def viridis(cls, field, min_value, max_value, opacity=1, lic=False):
        """
    ...
    @classmethod
    def magma(cls, field, min_value, max_value, opacity=1, lic=False):
        """
    ...
    @classmethod
    def airflow(cls, field, min_value, max_value, opacity=1, lic=False):
        """
