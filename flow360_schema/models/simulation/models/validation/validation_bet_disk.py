...
def _check_bet_disk_initial_blade_direction_and_blade_line_chord(bet_disk):
    if bet_disk.blade_line_chord > 0 and bet_disk.initial_blade_direction is None:
        raise ValueError(
            "the initial_blade_direction" " is required to specify since its blade_line_chord is non-zero."
        )
    if bet_disk.initial_blade_direction is not None and bet_disk.blade_line_chord == 0:
        raise ValueError("the blade_line_chord has to be positive" " since its initial_blade_direction is specified.")
    return bet_disk
...
def _check_3d_coeffs_in_BET_polars(coeffs_3d, num_Mach, num_Re, num_alphas, section_index, coeffs_name):
