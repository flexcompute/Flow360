...
VelocityDirectionType = Union[tuple[StringExpression, StringExpression, StringExpression], Axis]
...
class Inflow(BoundaryBaseWithTurbulenceQuantities):
    ...
    velocity_direction: VelocityDirectionType | None = pd.Field(
        None,
        description="Direction of the incoming flow, pointing into the volume. Either a "
        + "constant unit vector, or a tuple of three expressions in the mesh coordinates "
        + "`x`, `y`, `z` for a spatially-varying direction. An expression direction need not "
        + "be normalized; it is normalized per node by the solver. If unspecified, the "
...
class SlipWall(BoundaryBase):
    ...
    entities: EntityList[Surface, MirroredSurface, WindTunnelGhostSurface, GhostCircularPlane, GhostSphere] = pd.Field(
        alias="surfaces",
...
class SymmetryPlane(BoundaryBase):
    ...
    entities: EntityList[Surface, MirroredSurface, GhostCircularPlane, WindTunnelGhostSurface] = pd.Field(
        alias="surfaces",
...
class Periodic(Flow360BaseModel):
    ...
    entity_pairs: UniqueItemList[SurfacePair | GhostSurfacePair] = pd.Field(
        alias="surface_pairs",
        description="List of matching pairs of :class:`~flow360.Surface` or `~flow360.GhostCircularPlane`. ",
    )
    ...
    @classmethod
    def _ensure_no_zone_zone_interface(cls, value):
        """A zone-zone interface is already paired with its coincident donor face and
        cannot additionally be paired with a periodic image face."""
        for surface_pair in value.items:
            for surface in surface_pair.pair:
                if isinstance(surface, Surface) and surface.private_attribute_is_interface:
                    raise ValueError(f"Zone-zone interface `{surface.name}` cannot be used in `Periodic`.")
        return value
