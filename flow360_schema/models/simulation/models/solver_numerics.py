...
class NavierStokesSolver(GenericSolverSettings):
    ...
    ...     riemann_solver=fl.RoeFlux(numerical_dissipation_factor=0.01, low_mach_preconditioner=True),
    ...
    @classmethod
    @deprecation_reminder("26.99.999")
    def _move_legacy_roe_fields_to_riemann_solver(cls, data):
        """Move Python legacy Navier-Stokes Roe knobs onto ``RoeFlux``.

        Serialized dictionaries are migrated by the version updater instead.
        """
        ...
        legacy_values = {field: data[field] for field in roe_only_fields if field in data}
        if not legacy_values:
            return data

        riemann_solver = data.get("riemann_solver")
        if riemann_solver is None:
            riemann_solver = RoeFlux()
        elif isinstance(riemann_solver, SLAU2Flux):
            raise ValueError(
                f"Fields {sorted(legacy_values)} are Roe-specific and cannot be set "
                "on NavierStokesSolver when riemann_solver is SLAU2Flux."
            )
        elif not isinstance(riemann_solver, RoeFlux):
            return data

        for field, ns_value in legacy_values.items():
            if field in riemann_solver.model_fields_set and getattr(riemann_solver, field) != ns_value:
                raise ValueError(
                    f"{field} is set on both NavierStokesSolver ({ns_value}) and RoeFlux "
                    f"({getattr(riemann_solver, field)}). Set it on RoeFlux only."
                )

        data = dict(data)
        for field in legacy_values:
            data.pop(field)
        data["riemann_solver"] = RoeFlux(**{**riemann_solver.model_dump(), **legacy_values})
        logger.warning(
            f"Setting {sorted(legacy_values)} on NavierStokesSolver is deprecated; "
            "set them on the RoeFlux riemann_solver instead."
        )
        return data
