...
class LinearSolver(Flow360BaseModel):
    ...
    absolute_tolerance: PositiveFloat | None = pd.Field(
        None,
    ...
    relative_tolerance: PositiveFloat | None = pd.Field(
        None,
    ...
    def _check_tolerance_conflict(self) -> Self:
        if self.absolute_tolerance is not None and self.relative_tolerance is not None:
            raise ValueError("absolute_tolerance and relative_tolerance cannot be specified at the same time.")
        return self
...
class KrylovLinearSolver(LinearSolver):
    ...
    ...     max_iterations=15,
    ...     max_preconditioner_iterations=15,
    ...     relative_tolerance=0.05,
    ... )
    """

    type_name: Literal["KrylovLinearSolver"] = pd.Field("KrylovLinearSolver", frozen=True)
    max_iterations: pd.conint(gt=0, le=50) = pd.Field(15, description="Number of Krylov iterations.")
    max_preconditioner_iterations: PositiveInt = pd.Field(
        15, description="Number of preconditioner sweeps per Krylov iteration."
    )
...
class RoeFlux(Flow360BaseModel):
    ...
    low_mach_preconditioner_threshold: NonNegativeFloat | None = pd.Field(
        None,
...
class SLAU2Flux(Flow360BaseModel):
    ...
    Reference: Kitamura & Shima, J. Comput. Phys., Vol. 245, pp. 62-83, 2013.
    DOI: https://doi.org/10.1016/j.jcp.2013.02.046

    SLAU2 is applied to interior face fluxes only. Boundary fluxes (freestream,
    supersonic inflow, wall, etc.) continue to use the Roe flux for robustness,
    even when SLAU2 is selected for the interior.
    ...
    jacobian: Literal["Roe", "SLAU2"] = pd.Field(
        "SLAU2",
        description='Jacobian used by the implicit solver. "SLAU2" (default) is '
        + 'consistent with the SLAU2 residual. "Roe" uses the Roe Jacobian, which can '
        + "be more robust for low-speed flows where the SLAU2 Jacobian struggles.",
    )
...
class NavierStokesSolver(GenericSolverSettings):
    ...
    CFL_multiplier: PositiveFloat = pd.Field(
        1.0,
        description="Factor to the CFL definitions defined in the " + ":ref:`Time Stepping <timeStepping>` section.",
    )
    ...
    riemann_solver: RiemannSolverType = pd.Field(
        ...
        description="Riemann solver used for the inviscid flux. Defaults to :class:`RoeFlux`.",
    )
    ...
    def _validate_line_search(self) -> Self:
        if self.line_search is not None and not isinstance(self.linear_solver, KrylovLinearSolver):
            raise ValueError("line_search can only be set when linear_solver is a KrylovLinearSolver.")
        return self

    @pd.model_validator(mode="after")
    def _validate_roe_specific_fields(self) -> Self:
        """Roe-specific dissipation knobs may be set legacy-style on this class or
        idiomatically on a :class:`RoeFlux` riemann_solver.

        - When ``riemann_solver`` is :class:`SLAU2Flux`, setting any of these fields
          to a non-default value on this class raises a validation error.
        - When ``riemann_solver`` is :class:`RoeFlux`, setting the same field to
          conflicting non-default values on both places raises. A non-default value set
          only on the RoeFlux is mirrored onto this class so the wire format keeps working.

        We compare against the field defaults instead of inspecting ``model_fields_set``
        because preprocessing reconstructs the model from a full ``model_dump``, which
        marks every field as 'set' regardless of user intent.
        """
        roe_only_fields = (
        ...
        def _non_default(model, name: str) -> bool:
            return getattr(model, name) != type(model).model_fields[name].default

        non_default_on_ns = {f for f in roe_only_fields if _non_default(self, f)}

        if isinstance(self.riemann_solver, SLAU2Flux) and non_default_on_ns:
            raise ValueError(
                f"Fields {sorted(non_default_on_ns)} are Roe-specific and cannot be set "
                f"on NavierStokesSolver when riemann_solver is SLAU2Flux."
            )

        if isinstance(self.riemann_solver, RoeFlux):
            non_default_on_roe = {f for f in roe_only_fields if _non_default(self.riemann_solver, f)}
            for f in non_default_on_ns & non_default_on_roe:
                ns_val = getattr(self, f)
                roe_val = getattr(self.riemann_solver, f)
                if ns_val != roe_val:
                    raise ValueError(
                        f"{f} is set on both NavierStokesSolver ({ns_val}) and RoeFlux "
                        f"({roe_val}). Set it in one place only."
                    )
            for f in non_default_on_roe - non_default_on_ns:
                # Mirror RoeFlux value to NavierStokesSolver so the wire format keeps working.
                setattr(self, f, getattr(self.riemann_solver, f))

            # Soft-deprecate NS-level Roe knobs: the canonical home is RoeFlux.
            # Only warn when the user actually set a non-default value here and didn't
            # already set it on RoeFlux (i.e. they're using the legacy entry point).
            legacy_only = non_default_on_ns - non_default_on_roe
            for f in legacy_only:
                # Mirror legacy NS-level value onto RoeFlux so the nested riemannSolver
                # representation matches the top-level (legacy) one in the dumped JSON.
                setattr(self.riemann_solver, f, getattr(self, f))
            if legacy_only:
                warnings.warn(
                    f"Setting {sorted(legacy_only)} on NavierStokesSolver is deprecated; "
                    f"set them on the RoeFlux riemann_solver instead, e.g. "
                    f"NavierStokesSolver(riemann_solver=RoeFlux({sorted(legacy_only)[0]}=...))",
                    DeprecationWarning,
                    stacklevel=2,
                )
...
TurbulenceModelConstants = Annotated[
    SpalartAllmarasModelConstants | KOmegaSSTModelConstants,
    pd.Field(discriminator="type_name"),
]
...
class TurbulenceModelControls(Flow360BaseModel):
    ...
    modeling_constants: TurbulenceModelConstants | None = pd.Field(
        None,
    ...
    enforcement: Literal["RANS", "LES"] | None = pd.Field(
...
class TurbulenceModelSolver(GenericSolverSettings, metaclass=ABCMeta):
    ...
    type_name: str = pd.Field(description=":code:`SpalartAllmaras`, :code:`kOmegaSST`, or :code:`None`.")
    absolute_tolerance: PositiveFloat = pd.Field(
        1e-8,
    ...
    hybrid_model: DetachedEddySimulation | None = pd.Field(
        None, description="Model used for running hybrid RANS-LES simulations"
    )

    rotation_correction: bool = pd.Field(False, description="Rotation correction for the turbulence model.")

    controls: list[TurbulenceModelControls] | None = pd.Field(
        None,
        strict=True,  # Note: To ensure propoer err msg when none-list is fed.
        description="List of control zones to enforce specific turbulence model constants " + "and behavior.",
    )
    ...
    def _check_zonal_modeling_constants_consistency(self) -> Self:
        ...
        for index, control in enumerate(self.controls):
            if control.modeling_constants is None:
                continue
            if not isinstance(control.modeling_constants, SpalartAllmarasModelConstants) and isinstance(
                self, SpalartAllmaras
            ):
                raise ValueError(
            ...
            if not isinstance(control.modeling_constants, KOmegaSSTModelConstants) and isinstance(self, KOmegaSST):
                raise ValueError(
...
class SpalartAllmaras(TurbulenceModelSolver):
    ...
    modeling_constants: SpalartAllmarasModelConstants | None = pd.Field(
    ...
    reconstruction_gradient_limiter: pd.confloat(ge=0, le=2) | None = pd.Field(
        0.5,
        description="The strength of gradient limiter used in reconstruction of solution "
        + "variables at the faces (specified in the range [0.0, 2.0]). 0.0 corresponds to "
        + "setting the gradient equal to zero, and 2.0 means no limiting.",
    )
    low_reynolds_correction: bool | None = pd.Field(
        False,
...
TurbulenceModelSolverType = Annotated[NoneSolver | SpalartAllmaras | KOmegaSST, pd.Field(discriminator="type_name")]
...
class TransitionModelSolver(GenericSolverSettings):
    ...
    def _check_n_crit_conflict(self) -> Self:
        if self.N_crit is not None and self.turbulence_intensity_percent is not None:
            raise ValueError("N_crit and turbulence_intensity_percent cannot be specified at the same time.")
        return self

    type_name: Literal["AmplificationFactorTransport"] = pd.Field("AmplificationFactorTransport", frozen=True)
    CFL_multiplier: PositiveFloat = pd.Field(
        2.0,
        description="Factor to the CFL definitions defined in the " + ":ref:`Time Stepping <timeStepping>` section.",
    )
    ...
    turbulence_intensity_percent: pd.confloat(ge=0.03, le=2.5) | None = pd.Field(
        None,
        description=":ref:`Turbulence Intensity <TurbI>`, Range from [0.03-2.5]. "
        + "Only valid when :py:attr:`N_crit` is not specified.",
    )
    N_crit: pd.confloat(ge=1.0, le=11.0) | None = pd.Field(
        None,
    ...
    reconstruction_gradient_limiter: pd.confloat(ge=0.0, le=2.0) | None = pd.Field(
        1.0,
        description="The strength of gradient limiter used in reconstruction of solution "
        + "variables at the faces (specified in the range [0.0, 2.0]). 0.0 corresponds to "
        + "setting the gradient equal to zero, and 2.0 means no limiting.",
    ...
    trip_regions: EntityList[Box] | None = pd.Field(
        None,
        description="A list of :class:`~flow360.Box` entities defining the trip zones.",
    )
    ...
    def _set_aft_ncrit(self) -> Self:
        ...
        if self.turbulence_intensity_percent is not None:
            ncrit_converted = -8.43 - 2.4 * np.log(0.025 * np.tanh(self.turbulence_intensity_percent / 2.5))
...
TransitionModelSolverType = Annotated[NoneSolver | TransitionModelSolver, pd.Field(discriminator="type_name")]
