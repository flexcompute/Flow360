...
def parse_in_xfoil_polar(polar_file_content: str):
    ...
    alphas = (
        list(np.arange(-180, -30, 10).astype(float)) + deg_increment_ang + list(np.arange(30, 190, 10).astype(float))
    )
...
def blend_polars_to_flat_plate(cl_alphas, cl_mach_nums, cl_values, cd_values):
    ...
    for _ in range(num_missing_alphas_max - 1):
        ...
        for _, mach in enumerate(cl_mach_nums):
            ...
            cl = cl_values[mach][-1] * blend_val + (1 - blend_val) * cos(a) * 2 * pi * sin(a) / sqrt(
                1 + (2 * pi * sin(a)) ** 2
            )
            cd = (
...
def read_in_c81_polars(
    ...
    for sec_idx, _ in enumerate(bet_disk["sectional_radiuses"]):
        ...
        if c81_polar_file_extensions[sec_idx].lower() == ".csv":
            alpha_list, mach_list, cl_list, cd_list = read_in_c81_polar_csv(polar_file_content)
        else:
            alpha_list, mach_list, cl_list, cd_list = read_in_c81_polar_c81_format(polar_file_content)
        if "mach_numbers" in bet_disk and bet_disk["mach_numbers"] != mach_list:
            raise Flow360ValueError(
        ...
        if "alphas" in bet_disk and bet_disk["alphas"] != alpha_list:
            raise Flow360ValueError(
...
def parse_c81_xfoil_geometry_file(
    ...
    for rad, tw, ch in zip(radius_station, twist, chord, strict=False):
...
def translate_xfoil_c81_to_bet_dict(
    ...
    if file_format == "xfoil":
        ...
        bet_disk = read_in_c81_polars(bet_disk, polar_file_contents_list, polar_file_extensions, angle_unit)
    return bet_disk
...
def read_xrotor_file(xrotor_file_content: str):
    ...
    for i in range(n_aero_sections):
        ...
        line_num += 1
        check_num_values(values, line_num, 4)
        xrotor_input_dict["dcldastall"][i] = abs(float(values[0]))  # make it absolute value to make sure it is positive
    ...
    xrotor_input_dict["omegaDim"] = xrotor_input_dict["vel"] / (xrotor_input_dict["adv"] * xrotor_input_dict["rad"])
    xrotor_input_dict["RPM"] = xrotor_input_dict["omegaDim"] * 30 / pi
    xrotor_input_dict["inputType"] = "xrotor"
    return xrotor_input_dict


def parse_xrotor_dfdc_geometry_file(geometry_file_content: str, file_format: Literal["xrotor", "dfdc"]):
    """
...
def generate_alphas():
    ...
    pos_ang = np.concatenate([np.arange(-9, -1, 0.5), np.arange(-1, 3, 0.25), np.arange(3, 9.5, 0.5)])
...
def blend_func_value(blend_window, alpha, alpha_min_max, alpha_range):
    ...
    if "below_cl_min" in alpha_range:
        ...
        if alpha < alpha_min_max - blend_window:
            return 0
        return cos((alpha - alpha_min_max) / blend_window * pi / 2) ** 2
    raise Flow360ValueError(f"alpha_range must be either above_cl_max or below_cl_min, it is: {alpha_range}")
...
def calc_cl_cd(xrotor_dict, alphas, mach_num, nrR_station):
    ...
    if msq > 1.0:
        logger.warning("CLFUNC: Local Mach^2 number limited to 0.99, was %s", msq)
        msq = 0.99
...
def translate_xrotor_dfdc_to_bet_dict(
    ...
    bet_disk["twists"] = generate_twists(xrotor_dict, length_unit=length_unit, angle_unit=angle_unit)
    ...
    bet_disk["reynolds_numbers"] = generate_reynolds()
    bet_disk["sectional_radiuses"] = [bet_disk["radius"] * r for r in xrotor_dict["rRstations"]] * length_unit
    bet_disk["sectional_polars"] = []
