...
from flow360_schema.models.simulation.outputs.output_fields import _FIELD_IS_SCALAR_MAPPING
from flow360_schema.models.simulation.outputs.outputs import (
    MONITOR_OUTPUT_TYPE_NAMES,
    MonitorOutputTypes,
    ProbeOutput,
...
class StoppingCriterion(Flow360BaseModel):
    ...
    name: str | None = pd.Field("StoppingCriterion", description="Name of this criterion.")
    monitor_field: UserVariable | str = pd.Field(
        description="The field to be monitored. This field must be "
        "present in the `output_fields` of `monitor_output`."
    )
    monitor_output: str = pd.Field(
        description="The monitored output, given as the object or its id; stored as the id. "
        "Must reference a `ProbeOutput`, `SurfaceProbeOutput`, `SurfaceIntegralOutput` "
        "or `ForceOutput` in `outputs`."
    )
    tolerance: ValueOrExpression[UnytQuantity | float] = pd.Field(
        description="The tolerance threshold of this criterion."
    )
    tolerance_window_size: int | None = pd.Field(
        None,
    ...
    @classmethod
    def _serialize_monitor_output_to_id(cls, value):
        """Accept a monitor output object and store only the id (the wire format)."""
        if isinstance(value, Flow360BaseModel):
            if not isinstance(value, MonitorOutputTypes):
                raise ValueError(
                    f"`{type(value).__name__}` cannot be used as the monitor output of a stopping "
                    f"criterion. Supported monitor outputs: {MONITOR_OUTPUT_TYPE_NAMES}."
                )
            return value.private_attribute_id
        return value

    @contextual_field_validator("monitor_output", mode="after", required_context=["output_dict"])
    @classmethod
    def _check_monitor_output_reference(cls, v, param_info: ParamsValidationInfo):
        """Ensure the id resolves to an output in SimulationParams of an allowed type."""
        ...
        if not isinstance(monitor_output, MonitorOutputTypes):
            raise ValueError(
                f"`{monitor_output.output_type}` cannot be used as the monitor output of a stopping "
                f"criterion. Supported monitor outputs: {MONITOR_OUTPUT_TYPE_NAMES}."
            )
        return v
    ...
    def _check_not_final_pseudo_step_only_in_steady(cls, v, param_info: ParamsValidationInfo):
        ...
        if param_info.time_stepping != TimeSteppingType.STEADY:
            return v
        monitor_output = param_info.output_dict.get(v)
        if monitor_output is not None and getattr(monitor_output, "output_at_final_pseudo_step_only", False):
            raise ValueError(
    ...
    @contextual_field_validator("monitor_output", mode="after", required_context=["output_dict"])
    @classmethod
    def _check_field_exists_in_monitor_output(cls, v, info: pd.ValidationInfo, param_info: ParamsValidationInfo):
    ...
    @classmethod
    def _preprocess_field_with_unit_system(cls, value, info: pd.ValidationInfo, param_info: ParamsValidationInfo):
        if is_variable_with_unit_system_as_units(value):
            return value
        if info.data.get("monitor_field") is None:
            # `field` validation failed.
            raise ValueError("The monitor field is invalid and therefore unit inference is not possible.")
        if info.data.get("monitor_output") is None:
            raise ValueError("The monitor output is invalid and therefore unit inference is not possible.")
        units = value["units"]
        ...
        value = infer_units_by_unit_system(value=value, value_dimensions=field_dimensions, unit_system=units)
        return value
