...
from flow360_schema.framework.expression import Expression, StringExpression, ValueOrExpression
...
class ThermalStateCache(Flow360BaseModel):
    ...
    altitude: Length.Float64 | None = None
    temperature_offset: DeltaTemperature.Float64 | None = None
...
class GenericReferenceConditionCache(Flow360BaseModel):
    ...
    mach: pd.PositiveFloat | None = None
...
class GenericReferenceCondition(MultiConstructorBaseModel):
    ...
    velocity_magnitude: ValueOrExpression[Velocity.PositiveFloat64] | None = ConditionalField(
        context=CASE,
    ...
    def mach(self) -> pd.PositiveFloat:
        ...
        return float((self.velocity_magnitude / self.thermal_state.speed_of_sound).value)
...
class AerospaceConditionCache(Flow360BaseModel):
    ...
    mach: pd.NonNegativeFloat | None = None
    reynolds_mesh_unit: pd.PositiveFloat | None = None
    project_length_unit: Length.PositiveFloat64 | None = None
    alpha: Angle.Float64 | None = None
    beta: Angle.Float64 | None = None
    temperature: AbsoluteTemperature.Float64 | None = None
    thermal_state: ThermalState | None = pd.Field(None, alias="atmosphere")
    reference_mach: pd.PositiveFloat | None = None
...
class AerospaceCondition(MultiConstructorBaseModel):
    ...
    velocity_magnitude: ValueOrExpression[Velocity.NonNegativeFloat64] | None = ConditionalField(
        description="Freestream velocity magnitude. Used as reference velocity magnitude"
    ...
    reference_velocity_magnitude: Velocity.PositiveFloat64 | None = CaseField(
        None,
    ...
    def from_mach(
        ...
        alpha: Angle.Float64 = 0 * u.deg,
        beta: Angle.Float64 = 0 * u.deg,
        thermal_state: ThermalState | None = None,
        reference_mach: pd.PositiveFloat | None = None,
    ):
        """
        ...
        reference_velocity_magnitude = reference_mach * thermal_state.speed_of_sound if reference_mach else None
    ...
    def from_mach_reynolds(
        ...
        project_length_unit: Length.PositiveFloat64 | None,
        alpha: Angle.Float64 = 0 * u.deg,
        beta: Angle.Float64 = 0 * u.deg,
        ...
        if temperature.units is u.K and temperature.value == 288.15:
            logger.info("Default value of 288.15 K will be used as temperature.")
        ...
        density = reynolds_mesh_unit * material.get_dynamic_viscosity(temperature) / (velocity * project_length_unit)
    ...
    def _evaluated_velocity_magnitude(self) -> Velocity.PositiveFloat64:
        if isinstance(self.velocity_magnitude, Expression):
            return self.velocity_magnitude.evaluate(raise_on_non_evaluable=True, force_evaluate=True)
    ...
    def mach(self) -> pd.PositiveFloat:
        """Computes Mach number."""
        return float((self._evaluated_velocity_magnitude / self.thermal_state.speed_of_sound).value)
    ...
    @pd.validate_call
    def flow360_reynolds_number(self, length_unit: Length.PositiveFloat64):
        """
...
class LiquidOperatingCondition(Flow360BaseModel):
    ...
    type_name: Literal["LiquidOperatingCondition"] = pd.Field("LiquidOperatingCondition", frozen=True)
    alpha: Angle.Float64 = ConditionalField(0 * u.deg, description="The angle of attack.", context=CASE)
    beta: Angle.Float64 = ConditionalField(0 * u.deg, description="The side slip angle.", context=CASE)
    velocity_magnitude: ValueOrExpression[Velocity.NonNegativeFloat64] | None = ConditionalField(
        context=CASE,
...
OperatingConditionTypes = Union[GenericReferenceCondition, AerospaceCondition, LiquidOperatingCondition]
