...
def validate_model(
    *,
    ...
    version_to: str = _SCHEMA_PACKAGE_VERSION,
    return_validation_info: Literal[False] = False,
...
def validate_model(
    *,
    ...
    validation_level: Literal["SurfaceMesh", "VolumeMesh", "Case", "All"] | list | None = ALL,
    version_to: str = _SCHEMA_PACKAGE_VERSION,
    return_validation_info: Literal[True],
) -> tuple[SimulationParams | None, list | None, list[dict[str, Any]], ParamsValidationInfo | None]: ...
...
def validate_model(
    *,
    ...
    With ``return_validation_info=True`` the result gains a fourth element: the
    ``ParamsValidationInfo`` used during validation (None when validation failed
    before it was built), whose registry and expansion cache callers may reuse.
    """
    ...
    def dict_preprocessing(params_as_dict: dict) -> tuple[dict, tuple]:
        params_as_dict = SimulationParams._sanitize_params_dict(params_as_dict)
        # Deserialize entity_info exactly once; the registry resolves compact entity
        # references and backs ParamsValidationInfo selector expansion, so both yield
        # the instances entity_info owns.
        entity_info_and_registry = ParamsValidationInfo._build_entity_info_and_registry(params_as_dict)
        # Materialization runs BEFORE the multi-constructor parser so compact
        # references inside private_attribute_input_cache (BETDiskCache.entities)
        # resolve to live instances before the recorded constructor re-runs. The
        # parser's own output stays inline (it dumps with the inline context flag)
        # and is parsed directly by model_validate. Disposable arrangement: the
        # variant-union redesign (PR #6080) retires the parser machinery entirely.
        materialize_entities_and_selectors_in_place(
            params_as_dict,
            entity_registry=entity_info_and_registry[1],
        )
        params_as_dict = handle_multi_constructor_model(params_as_dict)
        return params_as_dict, entity_info_and_registry

    validation_errors = None
    validation_warnings: list[dict[str, Any]] = []
    validated_param = None
    validation_context: ValidationContext | None = None
    validation_info: ParamsValidationInfo | None = None
    ...
    try:
        updated_param_as_dict, entity_info_and_registry = dict_preprocessing(params_as_dict)
        ...
        validation_info = ParamsValidationInfo(
            ...
            entity_info_and_registry=entity_info_and_registry,
        )
    ...
    if return_validation_info:
        return validated_param, validation_errors, validation_warnings, validation_info
