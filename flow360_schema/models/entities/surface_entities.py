...
Surface, GhostSphere, GhostCircularPlane, SurfacePair, Mirrored entities.
"""
...
class ImportedSurface(TransformableEntity, EntityBase):
    """ImportedSurface for post-processing.

    Convention: ``private_attribute_id`` is ``f"{name}_defaultBody"``. Handles are
    re-created from project dependency metadata (which persists no entity id)
    each session, and saved coordinate-system/mirror assignments are keyed by
    this id — so creators must derive it deterministically from the name.
    """
...
class WindTunnelGhostSurface(_SurfaceEntityBase):
...
class GhostSurfacePair(SurfacePairBase):
    ...
    Attributes:
        pair (Tuple[GhostCircularPlane, GhostCircularPlane]):
            A tuple containing two GhostCircularPlane objects representing the pair.
            Python-API accessors and the Web UI both produce (possibly partial)
            GhostCircularPlane instances; entity_info owns the geometry fields.
    """

    # Note: the retired pair type was a DISCRIMINATED union (GhostSurface | GhostCircularPlane),
    # which matched by tag — WindTunnelGhostSurface pairs were rejected then as now.
    pair: tuple[GhostCircularPlane, GhostCircularPlane]
