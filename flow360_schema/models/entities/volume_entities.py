...
@final
class Box(TransformableEntity, MultiConstructorBaseModel, _VolumeEntityBase):
    """
    ...
    def _canonicalize_rotation(self) -> Self:
        """Pin the rotation to one spelling so identical boxes serialize identically."""
        axis, angle_in_radians = canonicalize_axis_and_angle(
            self.axis_of_rotation,
            self.angle_of_rotation.to("rad").v.item(),  # type: ignore[attr-defined]
        )
        # Both fields are frozen, matching the "cannot change once specified" contract for
        # callers; canonicalizing is not a change of rotation, so write through the freeze.
        # Written back unconditionally: tuple equality would call (-0.0, 0, 1) equal to
        # (0.0, 0, 1) and leave the negative zero in place to serialize as "-0.0".
        object.__setattr__(self, "axis_of_rotation", axis)
        stored = self.angle_of_rotation.to("rad").v.item()  # type: ignore[attr-defined]
        # -0.0 == 0.0 is True in Python but the two serialize differently, so the sign of a
        # zero has to be compared separately or `from_principal_axes`'s -0.0 would survive.
        if angle_in_radians != stored or math.copysign(1.0, angle_in_radians) != math.copysign(1.0, stored):
            canonical = angle_in_radians * unyt.rad
            object.__setattr__(
                self,
                "angle_of_rotation",
                canonical.to(self.angle_of_rotation.units),  # type: ignore[attr-defined]
            )
        return self
    ...
    def _apply_transformation(self, matrix: NDArray) -> Self:
        ...
        # `model_copy` runs no validators, so canonicalize here too or a transformed box
        # escapes the single-spelling guarantee that `_canonicalize_rotation` gives.
        new_axis, new_angle = canonicalize_axis_and_angle(Axis(tuple(new_axis)), new_angle)
...
@final
class Sphere(TransformableEntity, _VolumeEntityBase):
    """
...
@final
class Cylinder(TransformableEntity, _VolumeEntityBase):
    """
...
@final
class AxisymmetricBody(TransformableEntity, _VolumeEntityBase):
    """
...
@final
class CustomVolume(TransformableEntity, _VolumeEntityBase):
...
@final
class SeedpointVolume(TransformableEntity, _VolumeEntityBase):
    """
