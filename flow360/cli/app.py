...
def flow360(ctx, profile, dev, uat, env):
    ...
    prev_profile = UserConfig.profile
    ...
    if profile is not None:
        ...
        def restore_profile():
            ...
            UserConfig.set_profile(prev_profile)
...
def login(ctx, profile, dev, uat, local, env, port, timeout):  # pylint: disable=too-many-arguments
    """
    ...
    try:
        ...
        result = wait_for_login(
            ...
            port=port,
            timeout=timeout,
            use_local_ui=cli_context.local,
...
def logout(ctx, profile, dev, uat, local, env):  # pylint: disable=too-many-arguments
    """
