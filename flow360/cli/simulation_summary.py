...
def _load_summary_dicts(simulation_params: dict) -> tuple[dict, dict, dict | None]:
    # WARNING: This deliberately bypasses the official deserializer (validate_model)
    # and uses the legacy constructor on a dict whose asset cache is stripped.
    # Summary is a read-only, terminal consumer, so lenient parsing is acceptable:
    # a variable_context that would fail validation degrades to placeholder values
    # instead of failing the whole summary. Compact entity references are resolved
    # explicitly below (against the asset cache, BEFORE it is stripped) since the
    # legacy constructor has no materializer.
    ...
    try:
        ...
        try:
            _, entity_registry = (
                ParamsValidationInfo._build_entity_info_and_registry(  # pylint: disable=protected-access
                    params_dict
                )
            )
        except (pd.ValidationError, ValueError):
            # Malformed project_entity_info: the read-only summary degrades
            # (unresolved reference groups summarize by their ids).
            entity_registry = None
        try:
            materialize_entities_and_selectors_in_place(
                params_dict, entity_registry=entity_registry
            )
        except (pd.ValidationError, ValueError):
            # Lenient by design: without a loadable entity_info (or with
            # undeserializable used_selectors), compact reference groups stay
            # unresolved and are summarized by their ids. A partial in-place
            # materialize leaves a hybrid dict, which the guarded SimulationParams
            # construction below degrades to the display-only summary.
            pass
        params_dict = _strip_private_cache(params_dict)
        # Display from the materialized dict: compact reference groups have been
        # resolved to entity instances there, so sampling shows real entity names.
        # The json round trip reduces live instances to name dicts.
        display_dict = json.loads(
            json.dumps(_strip_private_cache(copy.deepcopy(params_dict)), default=_display_label)
        )
        try:
            params = SimulationParams.load_lenient(copy.deepcopy(params_dict))
        except pd.ValidationError:
            # Unresolvable reference groups (no loadable entity_info): degrade
            # to the display-only summary instead of failing the read-only tool.
            return display_dict, display_dict, None
...
def _display_label(obj):
    """JSON fallback for materialized entity/selector instances in the display dict."""
...
def _compact_entity_collection(value):
    ...
    for key in _ENTITY_COLLECTION_KEYS:
        for entity in value.get(key) or []:
            if isinstance(entity, dict) and set(entity) == {"type", "ids"}:
                # Unresolved compact reference group (no registry available):
                # the ids are the best available labels.
                names.extend(entity["ids"])
            else:
...
def _entity_label(entity):
    ...
    # Materialized entity/selector instances carry their real name.
