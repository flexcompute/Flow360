...
class Flow360WebTimeoutError(Flow360WebError):
    """The gateway gave up on a webAPI call before the server answered it.

    Not a verdict: a long call can still finish server-side after the gateway stops
    waiting, so this error stays silent and the catch site decides what the user is
    told — or asks the cloud what actually happened.
    """

    logs_when_constructed = False
