...
from typing import Annotated, Any, Collection, List, Literal, Union, get_type_hints
...
class Flow360ConversionUnitSystem(pd.BaseModel):
    ...
    base_length: Annotated[float, pd.Field(np.inf), Flow360LengthUnit]
    base_mass: Annotated[float, pd.Field(np.inf), Flow360MassUnit]
    base_time: Annotated[float, pd.Field(np.inf), Flow360TimeUnit]
    base_temperature: Annotated[float, pd.Field(np.inf), Flow360TemperatureUnit]
    base_velocity: Annotated[float, pd.Field(np.inf), Flow360VelocityUnit]
    base_area: Annotated[float, pd.Field(np.inf), Flow360AreaUnit]
    base_force: Annotated[float, pd.Field(np.inf), Flow360ForceUnit]
    base_density: Annotated[float, pd.Field(np.inf), Flow360DensityUnit]
    base_pressure: Annotated[float, pd.Field(np.inf), Flow360PressureUnit]
    base_viscosity: Annotated[float, pd.Field(np.inf), Flow360ViscosityUnit]
    base_power: Annotated[float, pd.Field(np.inf), Flow360PowerUnit]
    base_moment: Annotated[float, pd.Field(np.inf), Flow360MomentUnit]
    base_angular_velocity: Annotated[float, pd.Field(np.inf), Flow360AngularVelocityUnit]
    base_heat_flux: Annotated[float, pd.Field(np.inf), Flow360HeatFluxUnit]
    base_heat_source: Annotated[float, pd.Field(np.inf), Flow360HeatSourceUnit]
    base_heat_capacity: Annotated[float, pd.Field(np.inf), Flow360HeatCapacityUnit]
    base_thermal_conductivity: Annotated[float, pd.Field(np.inf), Flow360ThermalConductivityUnit]
    base_inverse_area: Annotated[float, pd.Field(np.inf), Flow360InverseAreaUnit]
    base_inverse_length: Annotated[float, pd.Field(np.inf), Flow360InverseLengthUnit]

    registry: Any = pd.Field(frozen=True)
    conversion_system: Any = pd.Field(frozen=True)
    ...
    def __init__(self):
        ...
        for field_name, field_info in self.model_fields.items():
            if (
                hasattr(annotations[field_name], "__metadata__") == False
                or len(annotations[field_name].__metadata__) != 2
            ):
                continue
            target_dimension = annotations[field_name].__metadata__[1]
            registry.add(
    ...
    def assign_conversion_rate(cls, value, info: pd.ValidationInfo):
        ...
        if (
            hasattr(annotations[info.field_name], "__metadata__") == False
            or len(annotations[info.field_name].__metadata__) != 2
        ):
            return value

        target_dimension = annotations[info.field_name].__metadata__[1]
