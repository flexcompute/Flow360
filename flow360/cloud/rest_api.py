...
class RestApi:
    ...
    def post(self, json, path=None, method=None, compress_when_larger_than_mb=None):
        """
        Resource post
        """
        path = path or self._url(method)
        return self._http.post(
            self._api_url(path, self._environment_provider()),
            json=json,
            compress_when_larger_than_mb=compress_when_larger_than_mb,
        )

    def post_envelope(self, json, path=None, method=None) -> dict:
        """Resource post returning the full response envelope (code/data/detail)."""
        path = path or self._url(method)
        return self._http.post_json_envelope(
            self._api_url(path, self._environment_provider()), json=json
        )
