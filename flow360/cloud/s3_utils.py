...
from ..environment import Env, current_environment
...
class S3TransferType(Enum):
    ...
    def read_text(  # pylint: disable=too-many-arguments
        self,
        resource_id: str,
        remote_file_name: str,
        byte_range=None,
    ...
    def _get_s3_sts_token(self, resource_id: str, file_name: str) -> _S3STSToken:
        ...
        if session_key not in _s3_sts_tokens or _s3_sts_tokens[session_key].is_expired():
            from .rest_api import RestApi  # pylint: disable=import-outside-toplevel

            path = self._get_grant_url(resource_id, file_name)
            resp = RestApi(path, environment_provider=current_environment).get()
