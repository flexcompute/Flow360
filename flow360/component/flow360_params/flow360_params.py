...
def TurbulenceQuantities(
    ...
    nonNoneArgCount = sum(arg is not None for arg in locals().values())
    if nonNoneArgCount == 0:
        return None

    if viscosity_ratio is not None:
        if nonNoneArgCount == 1:
        ...
        if turbulent_intensity is not None:
            return TurbulentIntensityAndTurbulentViscosityRatio(
                turbulent_viscosity_ratio=viscosity_ratio, turbulent_intensity=turbulent_intensity
            )
    ...
    if modified_viscosity_ratio is not None and nonNoneArgCount == 1:
        return ModifiedTurbulentViscosityRatio(
            modified_turbulent_viscosity_ratio=modified_viscosity_ratio
        )

    if modified_viscosity is not None and nonNoneArgCount == 1:
    ...
    if turbulent_length_scale is not None and nonNoneArgCount == 1:
