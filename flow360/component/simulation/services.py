...
from flow360_schema.models.simulation.models.bet.charm_translator import (
    translate_charm_to_bet_dict,
)
...
def translate_charm_bet_disk(
    *,
    ...
    """
    Run the BET Disk translator for a CHARM blade geometry file.

    ``geometry_file_content`` is the CHARM blade geometry file (``*bg.inp``) and
    ``polar_file_contents_dict`` carries the CHARM airfoil tables file. Unlike
    XFOIL/C81, a CHARM blade geometry file does not reference its airfoil tables
    file by name -- only a CHARM master input does -- so exactly one entry is
    required here and its name is not validated against the geometry file.
    Returns the dict of BETDisk.
    """
    errors = []
    bet_dict_list = []
    try:
        length_unit = _validate_unit_string(length_unit, "length")
        angle_unit = _validate_unit_string(angle_unit, "angle")
        if len(polar_file_contents_dict) != 1:
            raise ValueError(
                "Exactly one CHARM airfoil tables file is expected, but "
                f"{len(polar_file_contents_dict)} were uploaded: "
                f"{sorted(polar_file_contents_dict.keys())}."
            )
        (airfoil_tables_file_content,) = polar_file_contents_dict.values()
        bet_disk_dict = translate_charm_to_bet_dict(
            bg_file_content=geometry_file_content,
