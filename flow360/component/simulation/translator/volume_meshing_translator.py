        ...
        if isinstance(zone, CustomZones):
            ...
            for custom_volume in zone.entities.stored_entities:
                if isinstance(custom_volume, CustomVolume):
                    zone_name = custom_volume.name
                    patch_names = []
                    for surface in custom_volume.boundaries.stored_entities:
                        if type(surface) in (GhostSurface, GhostSphere, GhostCircularPlane):
                            # AutomatedFarfield ghost entities (GhostSurface => GhostSphere/GhostCircularPlane
                            # after expansion) will be auto-generated by the mesher and should not appear
                            # in the patches list. Do not filter out WindTunnelGhostSurfaces.
                            if surface.name == "farfield":
                                zone_name = "farfield"
                            continue
                        patch_names.append(surface.name)
                    volume_dict = {
                        "name": zone_name,
