    ...
    def assign(
        ...
        *,
        ...
        for entity in normalized_entities:
            ...
            if not isinstance(entity, TransformableEntity):
                raise Flow360ValueError(
                    f"A coordinate system cannot be assigned to '{entity.name}' "
                    f"({entity.private_attribute_entity_type_name}): that entity type carries no "
                    "transform, so the assignment would have no effect. For geometry surfaces and "
                    "edges, assign the coordinate system to the body group that owns them."
                )
