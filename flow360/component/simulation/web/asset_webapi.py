...
class DraftWebApi(ResourceWebApi):
    ...
    def post(
        self, json, path=None, method=None, compress_when_larger_than_mb=None
    ):  # pylint: disable=redefined-outer-name
        """Delegate specialized POST calls, optionally gzipping a large body.

        Only drafts carry the simulation.json envelope, so the other resource wrappers
        have no reason to reach the compression path.
        """
