...
def resolve_parent_folder(
    folder_or_workspace: Optional[Union[Folder, Workspace]], kwargs: dict, legacy_key: str
) -> Optional[Folder]:
    """Resolve a folder/workspace target; the legacy folder keyword is still accepted."""
    legacy_folder = kwargs.pop(legacy_key, None)
    if kwargs:
        raise TypeError(f"Unexpected keyword arguments: {sorted(kwargs)}")
    if folder_or_workspace is not None and legacy_folder is not None:
        raise Flow360ValueError(
            f"Specify either `{legacy_key}_or_workspace` or `{legacy_key}`, not both."
        )
    target = folder_or_workspace if folder_or_workspace is not None else legacy_folder
    if isinstance(target, Workspace):
        return Folder(id=target.root_folder_id)
    return target
...
class Folder(Flow360Resource):
    ...
    def create(
        ...
        name: str,
        tags: List[str] = None,
        parent_folder_or_workspace: Optional[Union[Folder, Workspace]] = None,
        **kwargs,
    ) -> FolderDraft:
        """Create a new folder
        ...
        parent_folder_or_workspace : Optional[Union[Folder, Workspace]], optional
            Parent location for the folder: a parent folder, or a workspace to create
            the folder in its root, e.g. ``fl.Workspace.get_shared()`` for the shared
            company workspace. If None, creates in the private workspace root.
            The legacy ``parent_folder=`` keyword is still accepted as an alias.

        Returns
        -------
        FolderDraft
            Draft folder; call ``.submit()`` to create it in the cloud.
        """
        parent_folder = resolve_parent_folder(parent_folder_or_workspace, kwargs, "parent_folder")
