...
class MultiConstructorModelBase(Flow360BaseModel, metaclass=abc.ABCMeta):
    private_attribute_constructor: Literal['default'] = pd.Field('default', frozen=True)
    private_attribute_input_cache: Any
    ...
    def model_constructor(cls, func: Callable) -> Callable:
        ...
        def wrapper(cls, *args, **kwargs):
            ...
            result.private_attribute_input_cache = result.__annotations__["private_attribute_input_cache"](
                **{**result.private_attribute_input_cache.model_dump(), **defaults, **kwargs}
            )
            with _model_attribute_unlock(result, 'private_attribute_constructor'):
                result.private_attribute_constructor = func.__name__
            return result
