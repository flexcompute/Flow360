...
def update_symmetric_ghost_entity_name_to_symmetry(*, params_as_dict: dict):
    """
    Recursively update ghost entity name from symmetric-* to symmetry-*
    """

    def recursive_update_symmetric_ghost_entity_name_to_symmetry(*, data):
        if isinstance(data, dict):
            # Check if current dict is an Entity item
            if (
                is_entity_item(data=data)
            ...
            for value in data.values():
                recursive_update_symmetric_ghost_entity_name_to_symmetry(data=value)

        elif isinstance(data, list):
            for element in data:
                recursive_update_symmetric_ghost_entity_name_to_symmetry(data=element)

    recursive_update_symmetric_ghost_entity_name_to_symmetry(data=params_as_dict)
