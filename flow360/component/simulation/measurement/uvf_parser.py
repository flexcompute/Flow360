"""UVF manifest parsing and triangle extraction.

Provides pure functions to parse UVF manifest JSON and extract face triangles
...
def extract_face_triangles(  # pylint: disable=too-many-locals
    manifest: List[dict],
    face_ids: List[str],
    bin_data_map: Dict[str, bytes],
    lod_level: Optional[int] = None,
) -> Dict[str, np.ndarray]:
    """Extract triangles grouped by face ID.

    Reads the UVF manifest structure to locate each face's triangle data within
    the binary buffers, supporting both indexed and unindexed geometry. The
    returned mapping preserves the order of ``face_ids``.
    ...
    triangles_by_face: Dict[str, List[np.ndarray]] = {face_id: [] for face_id in face_ids}
        ...
        if index_section is not None:
            ...
            for face in faces:
                for location in face["properties"]["bufferLocations"]["indices"]:
                    ...
                    triangles_by_face[face["id"]].append(
                        _reshape_triangles(face["id"], position_array[face_indices])
                    )
        else:
            ...
            for face in faces:
                for location in face["properties"]["bufferLocations"]["indices"]:
                    ...
                    triangles_by_face[face["id"]].append(
                        _reshape_triangles(face["id"], position_array[start_vertex:end_vertex])
                    )

    return {face_id: np.concatenate(triangles_by_face[face_id], axis=0) for face_id in face_ids}


def extract_face_vertices(
    manifest: List[dict],
    face_ids: List[str],
    bin_data_map: Dict[str, bytes],
    lod_level: Optional[int] = None,
    ...
    """
    triangles = extract_face_triangles(manifest, face_ids, bin_data_map, lod_level)
    return np.concatenate(list(triangles.values()), axis=0).reshape(-1, 3)


def _reshape_triangles(face_id: str, vertices: np.ndarray) -> np.ndarray:
    """Reshape a face buffer location into explicit triangles."""
    if len(vertices) % 3:
        raise ValueError(
            f"Face '{face_id}' has {len(vertices)} tessellation vertices; "
