"""Download and cache tessellation files for geometry measurements.

Two-layer cache strategy:
  L1: in-memory dict (instance-level, lives with the loader)
...
from .uvf_parser import extract_face_triangles, parse_manifest, resolve_buffers


class TessellationFileLoader:  # pylint: disable=too-few-public-methods
    """Downloads and caches tessellation files for geometry measurements.

    L1: in-memory cache (instance-level, lives with the loader)
    L2: disk cache via CloudFileCache (cross-session persistence)
    """
    ...
    def __init__(
        self,
        geometry_resources: Mapping[str, Flow360Resource],
    ...
    def load_vertices(self, face_ids: List[str], lod_level: Optional[int] = None) -> np.ndarray:
        ...
        Handles manifest loading, face-to-geometry mapping, bin downloading, and vertex extraction.
        ...
        """
        triangles = self.load_triangles(face_ids, lod_level)
        return np.concatenate(list(triangles.values()), axis=0).reshape(-1, 3)

    def load_triangles(
        self, face_ids: List[str], lod_level: Optional[int] = None
    ) -> Dict[str, np.ndarray]:
        """Extract triangles grouped by face ID across all geometries.

        Handles manifest loading, face-to-geometry mapping, bin downloading, and triangle
        extraction. The returned mapping preserves the order of ``face_ids``.

        Args:
            face_ids: Face IDs to extract triangles for.
            lod_level: LOD level override. None means use the manifest default.

        Returns:
            Mapping from face ID to an (N, 3, 3) float32 array of triangles.
        """
        ...
        triangles_by_face: Dict[str, np.ndarray] = {}
        for geometry_id, geo_face_ids in geometry_faces.items():
            ...
            triangles_by_face.update(
                extract_face_triangles(manifest, geo_face_ids, bin_data_map, lod_level)
            )

        return {face_id: triangles_by_face[face_id] for face_id in face_ids}
    ...
    def _ensure_manifests_loaded(self) -> None:
        ...
        if missing:
            log.info("Measurement: loading tessellation data from cloud...")
    ...
    def _ensure_face_index_built(self) -> None:
        ...
        Only indexes Face entries (type == "Face") to avoid collisions from
        shared structural IDs (e.g. root groups) across geometries.
        Raises on duplicate face IDs across geometries.
        """
    ...
    def _load_file(self, geometry_id: str, file_path: str) -> bytes:
        ...
        # Download from cloud
        log.debug(f"Measurement: downloading tessellation file '{file_path}' ({geometry_id})")
