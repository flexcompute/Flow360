...
class CustomVolume(_VolumeEntityBase):
    """
    CustomVolume is a volume zone defined by its enclosed entities.
    ...
    def ensure_unique_boundary_names(cls, v, param_info: ParamsValidationInfo):
        """Check if the enclosed entities have different names within a CustomVolume."""
        ...
        if len(expanded_surfaces) != len({entity.name for entity in expanded_surfaces}):
            raise ValueError("The enclosed entities of a CustomVolume must have different names.")
        return v
