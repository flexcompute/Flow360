...
class SymmetryPlane(BoundaryBase):
    ...
    def remap_symmetric_to_user_name(cls, value, param_info: ParamsValidationInfo):
        ...
        if not hasattr(value, "stored_entities") or not value.stored_entities:
            return value
        # Symmetry plane entity on the BC model (flows to solver JSON)
        bc_entity = next(
            (
                e
        ...
        if entity_info is None:
            return value
        # Symmetry plane entity in entity_info (used by asset boundary collection)
        ghost_asset = next(
            (
                g
        ...
        if len(sym_surfaces) == 1:
            ...
            for entity in (bc_entity, ghost_asset):
                if entity is None:
                    continue
