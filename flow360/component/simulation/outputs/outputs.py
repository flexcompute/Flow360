...
class _AnimationAndFileFormatSettings(_AnimationSettings):
    ...
    output_format: Literal["paraview", "tecplot", "both"] = pd.Field(default="paraview", description='"paraview", "tecplot" or "both".')
...
class SurfaceOutput(_AnimationAndFileFormatSettings):
    ...
    output_fields: UniqueAliasedStringList[SurfaceFieldNames] = pd.Field(description='List of output variables. Including :ref:`universal output variables<UniversalVariables>` and :ref:`variables specific to surfaceOutput<SurfaceSpecificVariables>`. :code:`outputFields` specified under :code:`surfaceOutput` will be added to all surfaces.')
...
class SurfaceIntegralOutput(Flow360BaseModel):
    ...
    name: str = pd.Field()
    entities: Optional[EntityList[Surface, GhostSurface]] = pd.Field(None, alias="surfaces",description='List of surface entities on which the surface integral will be calculated.')
    output_fields: UniqueAliasedStringList[CommonFieldNames] = pd.Field(description='List of output fields which will be added to all monitors within the monitor group, see universal output variables.')
...
class AeroAcousticOutput(Flow360BaseModel):
    ...
    write_per_surface_output: bool = pd.Field(False, description='Enable writing of aeroacoustic results on a per-surface basis, in addition to results for all wall surfaces combined.')
