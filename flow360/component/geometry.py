...
class GeometryDraft(ResourceDraft):
    ...
    def __init__(
        ...
        workflow : {"standard", "catalyst"}, optional
            Pipeline used for geometry processing. Use "catalyst" to route the
            geometry through the Nextflow-backed Catalyst pipeline (required
            for downstream GAI surface meshing). Defaults to "standard".
        """
    ...
    def _validate_geometry(self):
        ...
        if self.workflow not in GeometryWorkflow.__args__:
            raise Flow360ValueError(
                f"specified workflow : {self.workflow} is invalid. "
                f"Valid options are: {list(GeometryWorkflow.__args__)}"
            )
