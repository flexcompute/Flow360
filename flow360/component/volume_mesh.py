...
class VolumeMeshDraft(ResourceDraft):
    ...
    def _submit_from_surface(self, force_submit: bool = False):
        ...
        resp = RestApi(VolumeMeshInterface.endpoint, environment_provider=current_environment).post(
            body
        )
        if not resp:
            return None
...
class VolumeMesh(Flow360Resource):
    ...
    def copy_from_example(
        ...
        resp = RestApi(
            f"{VolumeMeshInterface.endpoint}/examples/copy",
...
class VolumeMeshV2(AssetBase):
    ...
    # pylint: disable=useless-parent-delegation
    def _apply_dynamic_default_settings(self, simulation_dict: dict):
        """Populate the default volume mesh settings from the simulation dict"""
        return super()._apply_dynamic_default_settings(simulation_dict)
