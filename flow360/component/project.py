...
from flow360.cloud.flow360_requests import CloneVolumeMeshRequest, LengthUnitType
...
def create_draft(
    *,
    ...
    # The entity_info property returns a fresh deserialization per access; that copy
    # is exclusively ours, so ownership transfers to the draft without another deep copy.
    entity_info_copy = new_run_from.entity_info

    if not isinstance(entity_info_copy, GeometryEntityInfo) and (
    ...
    return DraftContext(
        ...
        geometry_root=new_run_from if isinstance(new_run_from, Geometry) else None,
    )
...
class _Unset:  # pylint: disable=too-few-public-methods
    """Sentinel for ``update()`` arguments that were not provided (distinct from an explicit ``None``)."""


_UNSET = _Unset()


def _read_only_metadata_error(field: str) -> AttributeError:
    """Build the error raised when assigning to a read-only project metadata property."""
    return AttributeError(
        f"Project '{field}' is read-only; use project.update({field}=...) to change it."
    )
...
class Project(pd.BaseModel):
    ...
    @name.setter
    def name(self, value):
        raise _read_only_metadata_error("name")
    ...
    @tags.setter
    def tags(self, value):
        raise _read_only_metadata_error("tags")
    ...
    def description(self, value):
        raise _read_only_metadata_error("description")
    ...
    def _create_project_from_files(
        ...
        *,
        ...
        root_asset = draft.submit(run_async=run_async, description=description)
        if run_async:
            log.info(
        ...
        if isinstance(files, GeometryFiles):
            project._root_webapi = RestApi(
                GeometryInterface.endpoint,
                id=root_asset.id,
                environment_provider=current_environment,
            )
        elif isinstance(files, SurfaceMeshFile):
            project._root_webapi = RestApi(
                SurfaceMeshInterfaceV2.endpoint,
                id=root_asset.id,
                environment_provider=current_environment,
            )
        elif isinstance(files, VolumeMeshFile):
            project._root_webapi = RestApi(
                VolumeMeshInterfaceV2.endpoint,
                id=root_asset.id,
                environment_provider=current_environment,
            )
    ...
    def _create_project_from_volume_mesh_clone(
        ...
        *,
        ...
        resp = RestApi(
            VolumeMeshInterfaceV2.endpoint, environment_provider=current_environment
        ).post(req.dict(), method="clone")
    ...
    def from_geometry(
        ...
        tags: List[str] = None,
        run_async: bool = False,
        folder_or_workspace: Optional[Union[Folder, Workspace]] = None,
        workflow: GeometryWorkflow = "standard",
        ...
        **kwargs,
        ...
        folder_or_workspace : Optional[Union[Folder, Workspace]], optional
            Target location for the project: a parent folder, or a workspace to create
            the project in its root folder, e.g. ``fl.Workspace.get_shared()`` for the
            shared company workspace. If None, creates in the private workspace root.
            The legacy ``folder=`` keyword is still accepted as an alias.
        ...
        description : str, optional
            Project description (default is empty string).
        ...
        ... )
        """
        folder = resolve_parent_folder(folder_or_workspace, kwargs, "folder")
    ...
    def from_volume_mesh(
        ...
        **kwargs,
        ...
        if isinstance(file, VolumeMeshV2):
            # the file branch is covered when the asset draft is created
    ...
    def import_surface_mesh(
        ...
        The imported surface is a post-processing entity, also called a sample surface. It is
        not meshed and carries no boundary condition; the solution is interpolated onto it and
        it can only be assigned to `SurfaceOutput`, `TimeAverageSurfaceOutput` and
        `SurfaceIntegralOutput`. Register the returned handle on a draft through the
        ``imported_surfaces`` argument of `create_draft` before referencing it.

        Accepted formats are STL, CGNS and UGRID. A UGRID file requires its companion
        ``.mapbc`` file next to it, and the file must be uncompressed. Volume mesh files in
        UGRID or CGNS format are also accepted, in which case their boundary faces are used.
        Every patch in the file is merged into a single surface. The coordinates are consumed
        as stored, in the coordinate frame and length unit of the volume mesh, so a file
        authored in other units must be converted before import.
        ...
        name : str
            Name of the surface mesh dependency resource. This is also the key used to look the
            surface up on the draft, and the suffix used in the resulting output filenames.
        length_unit : LengthUnitType, optional
            Unit of length recorded on the resource (default is "m"). Note that this is not
            applied to the surface coordinates when the case runs.
        tags : list of str, optional
        ...
        Notes
        -----
        Nodes of an imported surface that do not fall inside the fluid domain, because they lie
        inside a solid body or outside the mesh, are written as ``NaN`` in a surface output and
        are excluded from a surface integral. The surface is never checked against the geometry
        or trimmed to fit, so an integral over a surface that clips the geometry silently covers
        only the part inside the fluid domain.
        """
        ...
        # Deterministic id: ImportedSurface handles are re-created from dependency
        # metadata each session, and saved assignments are keyed by this id.
        return ImportedSurface(
            name=name,
            private_attribute_id=f"{name}_defaultBody",
    ...
    def _get_dependency_resources_from_cloud(self, resource_type: ProjectDependencyType):
        ...
        if resource_type == ProjectDependencyType.GEOMETRY:
            ...
            imported_resources = [
                ImportedSurface(
                    name=item.name,
                    private_attribute_id=f"{item.name}_defaultBody",
    ...
    def _identify_unacknowledged_run(
        self, *, draft: Draft, target: AssetOrResource
    ) -> Optional[str]:
        """Which resource a run produced, when its acknowledgement never reached the client.

        The draft is the evidence that it ran at all: the cloud deletes it only once the
        run's resource exists, so a draft that goes away means the run went through. Which
        resource is then the newest of the target type — as close as a client can get,
        since the cloud identifies a run by a sequence number it never hands out. Returns
        None when the run cannot be pinned to a resource, having reported why — the two
        ways that happens mean different things to the user, while the submission ends the
        same way either way.
        """
        if not draft.confirm_run_started():
            log.error(
                "The cloud never confirmed the submission. It may still be processing, or it "
                "may have failed."
            )
            return None

        # pylint: disable=protected-access
        asset_type = target._cloud_resource_type_name
        try:
            records = self._project_webapi.get(method="tree")["records"]
        except Flow360Error as error:
            # An unreadable project is one more way of not finding the resource, not a
            # second kind of failure — and never a raw error thrown at a caller that asked
            # for a return value instead.
            records = []
            log.debug(f"Could not read the project while looking for the {asset_type}: {error}")
        newest = max(
            (record for record in records if record["type"] == asset_type),
            key=lambda record: parse_datetime(record["createdAt"]),
            default={},
        )
        if "id" not in newest:
            log.error(
                f"The submission went through, but the {asset_type} it created could not be "
                "found in this project."
            )
            return None

        log.warning(
            f"Returning the newest {asset_type} in this project. The submission did go through, "
            "but the cloud never said which resource it created, so check the name and creation "
            f"time to confirm this {asset_type} is the one submitted here."
        )
        return newest["id"]
    ...
    def update(
        ...
        *,
        ...
        """
        Update project metadata on the cloud.

        Only the arguments you pass are sent; omitted fields are left unchanged.
        Provided values (including ``None`` and an empty string) are forwarded
        verbatim and validated by the backend. The local metadata is refreshed
        afterwards.
        ...
        tags : list of str, optional
            New project tags.
        """
        provided = {"name": name, "description": description, "tags": tags}
        payload = {key: value for key, value in provided.items() if value is not _UNSET}
        self._project_webapi.patch(payload)
        self.metadata = ProjectMeta(**self._project_webapi.get())
    ...
    def _load_root_length_unit(self):
        """
        Reads the root asset's simulation JSON (via the asset's cached fetch) and
        ...
        simulation_json = AssetBase._get_simulation_json(
            asset=self._root_asset, clean_front_end_keys=True
        )
    ...
    def _run(
        ...
        **kwargs,
        ...
        # The uploaded document is the raw params dump, exactly as the WebUI submits;
        # validation runs remotely against the draft's own solver version — the
        # authoritative oracle — instead of this client's schema. Validator products
        # (defaulting/normalizing) are derived by the pipeline's validation at
        # translate time and never travel in the document.
        #
        # Nothing between here and /run leaves a draft behind: no run has been requested
        # yet, so a draft that cannot proceed is pure litter, and a user iterating on
        # their params would collect one per attempt. The /run failure path further down
        # deliberately keeps its draft instead, because a timeout there can mean the
        # server accepted the job anyway.
        try:
            draft.activate_dependencies(active_draft)
            draft.update_simulation_params(params)
            errors, warnings = draft.validate_simulation_json(
                root_item_type=self.metadata.root_item_type.value,
                up_to=target._cloud_resource_type_name,
            )
        except (RuntimeError, Flow360WebError) as error:
            log.error(f"Submission could not be completed: {error}")
            draft.delete_draft()
            if raise_on_error:
        ...
        if warnings:
            log.warning(
                f"Validation warnings found: {formatting_validation_warnings(warnings=warnings)}"
            )

        if errors is not None:
            log.error(f"Validation error found: {formatting_validation_errors(errors=errors)}")
            draft.delete_draft()
            if raise_on_error:
        ...
        try:
            ...
            destination_id = self._identify_unacknowledged_run(draft=draft, target=target)
            if destination_id is None:
                if raise_on_error:
                    raise ValueError(
                        "The cloud did not confirm which resource this submission created. "
                        "Check the project before submitting again."
                    ) from None
                return None
        ...
        **kwargs,
        ...
        if billing_method is not None and not draft_only:
            if billing_method == "FlexCredit":
                ...
                account_info = RestApi(
                    "flow360/account", environment_provider=current_environment
                ).get()
