...
class LinearResiduals(BaseChart2D):
    """
    Residuals is an object for showing the solution history of linear residuals.
    """
    ...
    section_title: Literal["Linear residuals"] = Field("Linear residuals", frozen=True)
    ...
    def set_x(self):
        self._x = (
            "linear_residuals/physical_step"
            if self.isUnsteady
            else "linear_residuals/pseudo_step"
        )
        return self
    ...
    _requirements: List[RequirementItem] = [
        RequirementItem.from_data_key(data_key="linear_residuals")
    ]
    # Internal tag 
    type_name: Literal["LinearResiduals"] = Field("LinearResiduals", frozen=True)
    ...
    def _handle_legend(self, cases, _, y_data):
        ...
        for case in cases:
            y_variables = [
                ...
                if res not in ("physical_step", "pseudo_step")
                and "iteration" not in res.lower()
            ]
    ...
    def _load_data(self, cases):
        cols_exclude = [
            col
            for col in cases[0].results.linear_residuals.as_dict().keys()
            if col in ("physical_step", "pseudo_step") or "iteration" in col.lower()
        ]
