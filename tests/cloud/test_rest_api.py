...
class _HttpRecorder:
    ...
    def post(self, path, json=None, compress_when_larger_than_mb=None):
        self.calls.append(
            (
                "post",
                path,
                {"json": json, "compress_when_larger_than_mb": compress_when_larger_than_mb},
            )
        )
        return {"method": "post"}
...
def test_rest_api_builds_absolute_urls_from_environment_provider():
    ...
    assert http.calls == [
        ...
        (
            ...
            {"json": {"name": "created"}, "compress_when_larger_than_mb": None},
        ),
        (
            "put",
            "https://api.example.test/v2/projects/1234567890123456/items",
