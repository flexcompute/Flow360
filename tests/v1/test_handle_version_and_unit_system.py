...
from flow360.component.v1xxx import Flow360Params
...
def test_import_no_unit_system_with_context():
    ...
    with pytest.raises(Flow360RuntimeError):
        with v1xxx.flow360_unit_system:
            Flow360Params(temp_file.name)
...
def test_import_with_unit_system_no_context():
    ...
    assert params
    assert params.unit_system == v1xxx.SI_unit_system
...
def test_create_no_unit_system_no_context():
    with pytest.raises(Flow360RuntimeError):
        v1xxx.Flow360Params(
            geometry=v1xxx.Geometry(
                ref_area=1 * u.m**2,
                moment_length=(1.47602, 0.801672958512342, 1.47602) * u.inch,
                moment_center=(1, 2, 3) * u.flow360_length_unit,
                mesh_unit=u.mm,
            ),
            fluid_properties=v1xxx.air,
            freestream=v1xxx.FreestreamFromVelocity(velocity=286 * u.m / u.s),
            time_stepping=v1xxx.UnsteadyTimeStepping(
                max_pseudo_steps=500,
                CFL=v1xxx.AdaptiveCFL(),
...
def test_create_with_unit_system_no_context():
    with pytest.raises(Flow360RuntimeError):
        v1xxx.Flow360Params(
            ...
            unit_system=v1xxx.SI_unit_system,
        )
...
def test_create_no_unit_system_with_context():
    with v1xxx.SI_unit_system:
        v1xxx.Flow360Params(
            ...
            boundaries={},
            fluid_properties=v1xxx.air,
            freestream=v1xxx.FreestreamFromVelocity(velocity=286),
            time_stepping=v1xxx.UnsteadyTimeStepping(
                max_pseudo_steps=500,
                CFL=v1xxx.AdaptiveCFL(),
                time_step_size=1.2 * u.s,
...
def test_parse_with_version():
    ...
    params = Flow360Params(temp_file.name)
    assert params.version == v1xxx.__version__
