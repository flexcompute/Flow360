...
def test_import_no_unit_system_with_context():
    ...
    with pytest.raises(Flow360RuntimeError):
        with modules.flow360_unit_system:
            Flow360Params(temp_file.name)
...
def test_import_with_unit_system_no_context():
    ...
    assert params
    assert params.unit_system == modules.SI_unit_system
...
def test_create_no_unit_system_no_context():
    with pytest.raises(Flow360RuntimeError):
        modules.Flow360Params(
            ...
            fluid_properties=modules.air,
            freestream=modules.FreestreamFromVelocity(velocity=286 * u.m / u.s),
            time_stepping=modules.UnsteadyTimeStepping(
                max_pseudo_steps=500,
                CFL=modules.AdaptiveCFL(),
...
def test_create_with_unit_system_no_context():
    with pytest.raises(Flow360RuntimeError):
        modules.Flow360Params(
            ...
            unit_system=modules.SI_unit_system,
        )
...
def test_create_no_unit_system_with_context():
    with modules.SI_unit_system:
        modules.Flow360Params(
            ...
            boundaries={},
            fluid_properties=modules.air,
            freestream=modules.FreestreamFromVelocity(velocity=286),
            time_stepping=modules.UnsteadyTimeStepping(
                max_pseudo_steps=500,
                CFL=modules.AdaptiveCFL(),
                time_step_size=1.2 * u.s,
...
def test_parse_with_version():
    ...
    params = Flow360Params(temp_file.name)
    assert params.version == modules.__version__
