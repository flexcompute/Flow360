...
def test_surface_output():
    ...
    with modules.SI_unit_system:
        params = Flow360Params(
            ...
            boundaries={
                "1": modules.NoSlipWall(name="wing"),
                "2": modules.SlipWall(name="symmetry"),
                "3": modules.FreestreamBoundary(name="freestream"),
            },
...
def test_slice_output():
    ...
    with pytest.raises(pd.ValidationError):
        output = SliceOutput(
            output_fields=["Cp", "qcriterion"],
            slices={
                "sliceName_1": modules.Slice(
                    slice_normal=(0, 1, 0),
                    slice_origin=(0, 0.56413, 0) * u.m,
                ),
                "sliceName_2": modules.Slice(
                    slice_normal=(0, 0, 1),
