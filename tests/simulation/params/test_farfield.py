...
from flow360_schema.models.entities.surface_entities import GhostCircularPlane, Surface
...
def test_user_defined_farfield_symmetry_plane(surface_mesh):
    ...
    assert (
        errors[0]["msg"]
        == "Value error, Specifying `domain_type` while starting from a surface mesh is only "
        "supported on `AutomatedFarfield`; the user-defined and wind-tunnel farfields use a "
        "symmetry wall you supply."
    )
    ...
    with SI_unit_system:
        params = SimulationParams(
            ...
            models=[
                Wall(surfaces=[s for s in surface_mesh["*"] if s.name != "preexistingSymmetry"]),
                SymmetryPlane(
                    surfaces=GhostCircularPlane(name="symmetric", private_attribute_id="symmetric")
                ),
            ],
        )
...
def test_rotated_symmetric_existence():
    ...
    def _test_and_show_errors(geometry):
        ...
        _, errors_1, warnings_1 = services.validate_model(
            params_as_dict=dump_wire_legal(processed_params),
            validated_by=services.ValidationCalledBy.LOCAL,
