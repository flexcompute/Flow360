...
from flow360_schema.models.entities.base import SurfacePrivateAttributes
from flow360_schema.models.entities.geometry_entities import Edge
from flow360_schema.models.entities.output_entities import Isosurface, Point, Slice
...
from flow360_schema.models.simulation.models.material import SolidMaterial, aluminum
...
from flow360_schema.testing import dump_wire_legal
...
def surface_output_with_wall_metric():
    surface_output = SurfaceOutput(
        name="surface",
        surfaces=[Surface(name="noSlipWall", private_attribute_id="noSlipWall")],
...
def fluid_model_with_low_mach_precond():
    return Fluid(
        navier_stokes_solver=NavierStokesSolver(
            riemann_solver=RoeFlux(low_mach_preconditioner=True),
        ),
    )
...
def fluid_model_with_low_numerical_dissipation():
    return Fluid(
        navier_stokes_solver=NavierStokesSolver(
            riemann_solver=RoeFlux(numerical_dissipation_factor=0.2),
        ),
    )
...
def test_BC_geometry():
    ...
    params, errors, _ = validate_model(
        params_as_dict=dump_wire_legal(params),
...
def test_incomplete_BC_volume_mesh():
    ...
    with SI_unit_system:
        params = SimulationParams(
            ...
            models=[
                ...
                SlipWall(
                    entities=[
                        Surface(name="plz_dont_do_this", private_attribute_id="plz_dont_do_this"),
                        no_bc,
                    ]
                ),
            ],
    ...
    assert len(errors) == 1
    # An unknown boundary now fails at reference resolution (accepted trade: the
    # friendlier aggregate message had no actionable fix either).
    assert "Entity reference not found in EntityRegistry" in errors[0]["msg"]
    assert "plz_dont_do_this" in errors[0]["msg"]
...
def test_incomplete_BC_surface_mesh():
    ...
    wall_1 = Surface(
        name="wall_1",
        private_attribute_id="wall_1",
        private_attribute_is_interface=False,
        private_attributes=dummy_attrs,
    )
    periodic_1 = Surface(
        name="periodic_1",
        private_attribute_id="periodic_1",
        private_attribute_is_interface=False,
        private_attributes=dummy_attrs,
    )
    periodic_2 = Surface(
        name="periodic_2",
        private_attribute_id="periodic_2",
        private_attribute_is_interface=False,
        private_attributes=dummy_attrs,
    )
    i_exist = Surface(
        name="i_exist",
        private_attribute_id="i_exist",
        private_attribute_is_interface=False,
        private_attributes=dummy_attrs,
    )
    no_bc = Surface(
        name="no_bc",
        private_attribute_id="no_bc",
        private_attribute_is_interface=False,
        private_attributes=dummy_attrs,
    )
    i_will_be_deleted = Surface(
        name="sym_boundary",
        private_attribute_id="sym_boundary",
        private_attribute_is_interface=False,
    ...
    asset_cache = AssetCache(
        ...
        project_entity_info=SurfaceMeshEntityInfo(
            ...
            ghost_entities=[
                GhostSphere(name="farfield", private_attribute_id="farfield"),
                GhostCircularPlane(
                    name="symmetric",
    ...
    # Deliberately NOT wire-legalized: plz_dont_do_this must stay unknown.
    params, errors, _ = validate_model(
...
def test_output_fields_with_user_defined_fields():
    surface_1 = Surface(name="some_random_surface", private_attribute_id="some_random_surface")
        ...
        params = SimulationParams(
            ...
            outputs=[
                ...
                TimeAverageSurfaceOutput(
                    ...
                    output_fields=["primitiveVars"],
                    entities=[
                        Surface(name="VOLUME/LEFT", private_attribute_id="VOLUME/LEFT"),
                    ],
                    start_step=4,
                    frequency=10,
                    frequency_offset=14,
                ...
                TimeAverageSurfaceOutput(
                    ...
                    output_fields=["T"],
                    entities=[
                        Surface(name="VOLUME/RIGHT", private_attribute_id="VOLUME/RIGHT"),
                    ],
                    start_step=4,
                    frequency=10,
                    frequency_offset=14,
...
def test_validate_liquid_operating_condition():
    ...
    with u.SI_unit_system:
        porous_zone = GenericVolume(
            name="zone_with_no_axes", private_attribute_id="zone_with_no_axes"
        )
        porous_zone.axes = [[0, 1, 0], [0, 0, 1]]
    # A GenericVolume zone only exists post-volume-mesh: use the faithful
    # VolumeMeshEntityInfo state (boundaries carry the interface flag there).
    all_boundaries = [
        surface.model_copy(update={"private_attribute_is_interface": False})
        for surface in all_boundaries
    ]
    asset_cache = AssetCache(
        project_length_unit=asset_cache.project_length_unit,
        project_entity_info=VolumeMeshEntityInfo(boundaries=all_boundaries, zones=[porous_zone]),
    )
    with u.SI_unit_system:
        pass
        params = SimulationParams(
            ...
            user_defined_dynamics=[
                UserDefinedDynamic(
                    ...
                    input_boundary_patches=[
                        Surface(name="UDDPatch", private_attribute_id="UDDPatch")
                    ],
                )
            ],
            outputs=[
    ...
    with u.SI_unit_system:
        params = SimulationParams(
            ...
            models=[
                Solid(
                    volumes=[GenericVolume(name="CHTSolid", private_attribute_id="CHTSolid")],
                    material=aluminum,
...
def test_beta_mesher_only_features(mock_validation_context):
        ...
        params = SimulationParams(
            meshing=MeshingParams(
                ...
                refinements=[
                    BoundaryLayer(
                        faces=[
                            Surface(name="face1", private_attribute_id="face1"),
                            Surface(name="face2", private_attribute_id="face2"),
                        ],
                        growth_rate=1.1,
                    )
                ],
    ...
    with SI_unit_system:
        surface1 = Surface(name="face1", private_attribute_id="face1")
        surface2 = Surface(name="face2", private_attribute_id="face2")
        params = SimulationParams(
            meshing=MeshingParams(
                defaults=MeshingDefaults(
                    boundary_layer_first_layer_thickness=1e-4,
                        ...
                        CustomZones(
                            ...
                            entities=[
                                ...
                                CustomVolume(
                                    name="zone1",
                                    bounding_entities=[
                                        Surface(name="face3", private_attribute_id="face3"),
                                        Surface(name="face4", private_attribute_id="face4"),
                                    ],
                                ),
                            ],
                        ),
    ...
    # `face1` bounds the CustomVolume, so it may turn out to be a zone-zone interface and is not
    # required to carry a boundary condition. `face3` belongs to no zone at all: `CustomZones`
    # makes the mesher build only the patches its zones list, so `face3` never reaches the volume
    # mesh and there is nothing for a boundary condition to apply to. Worth reporting, not worth
    # rejecting the submission over.
    assert errors is None
    assert any(
        warning["msg"]
        == (
            "The following boundaries are not part of any volume zone and will therefore not "
            "appear in the volume mesh: face3. If they are meant to be meshed, add them to the "
            "`bounding_entities` of a `CustomVolume` or to the `enclosed_entities` of the "
        ...
        params = SimulationParams(
            meshing=MeshingParams(
                ...
                refinements=[
                    GeometryRefinement(
                        geometry_accuracy=1e-5 * u.m,
                        entities=[Surface(name="noSlipWall", private_attribute_id="noSlipWall")],
                    )
                ],
            ),
            private_attribute_asset_cache=AssetCache(
    ...
    with SI_unit_system:
        params = SimulationParams(
            meshing=MeshingParams(
                ...
                refinements=[
                    SurfaceEdgeRefinement(
                        edges=[Edge(name="edge0001", private_attribute_id="edge0001")],
...
def test_check_custom_volume_in_volume_zones():
    from flow360_schema.models.simulation.meshing_param.volume_params import CustomZones

    zone_2 = CustomVolume(
        name="zone2", bounding_entities=[Surface(name="face2", private_attribute_id="face2")]
    )
    ...
    zone_3 = CustomVolume(
        name="zone3", bounding_entities=[Surface(name="face3", private_attribute_id="face3")]
    )
    zone_3.axis = (1, 0, 0)
...
def test_seedpoint_zone_based_params():
    ...
    with SI_unit_system:
        ...
        params = SimulationParams(
            ...
            models=[
                Wall(surfaces=[Surface(name="face1", private_attribute_id="face1")]),
                PorousMedium(
...
def test_auto_farfield_full_body_surface_on_y0_not_marked_deleted():
    surface_on_plane = Surface(
        name="plane_surf",
        private_attribute_id="plane_surf",
...
def test_auto_farfield_full_body_domain_type_surface_on_y0_not_marked_deleted():
    # Half-geometry sitting on +y side with a face on Y=0. Auto detection would treat
    # that face as a symmetry plane, but explicit `full_body` overrides detection so the
    # mesher keeps the full geometry and the face must not be flagged as deleted.
    surface_on_plane = Surface(
        name="plane_surf",
        private_attribute_id="plane_surf",
        private_attributes=SurfacePrivateAttributes(bounding_box=[[0, 0, 0], [1, 0, 1]]),
    ...
    asset_cache = AssetCache(
        ...
        use_geometry_AI=True,
        project_entity_info=SurfaceMeshEntityInfo(
            global_bounding_box=[[0, 0, 0], [1, 2, 1]],  # Half-geometry: one face on Y=0
            boundaries=[surface_on_plane],
            ghost_entities=[
                GhostSphere(
                    name="farfield",
    ...
    farfield = AutomatedFarfield(domain_type="full_body")
...
def test_auto_farfield_full_body_domain_type_no_symmetric_ghost_bc_warning():
    # AutomatedFarfield + full_body + GAI on a half-geometry whose bbox has a face on Y=0.
    # Auto detection would infer a symmetry plane, but explicit `full_body` tells the mesher
    # to keep the full geometry and generate no `symmetric` boundary. The `symmetric` ghost
    # must therefore not be treated as a real boundary needing a boundary condition.
    surface_body = Surface(
        name="body",
        private_attribute_id="body",
        private_attributes=SurfacePrivateAttributes(bounding_box=[[0, 0, 0], [1, 2, 1]]),
    )
    ...
    assert errors is None
    warning_msgs = [w["msg"] for w in (warnings or [])]
    assert not any("do not have a boundary condition" in msg for msg in warning_msgs), warning_msgs
...
def test_udf_symmetry_face_not_deleted():
    ...
    surface_sym = Surface(
        name="mySymmetry",
        private_attribute_id="mySymmetry",
        private_attributes=SurfacePrivateAttributes(bounding_box=[[0, 0, 0], [1, 0, 1]]),
    )
    surface_body = Surface(
        name="body",
...
def test_udf_symmetry_plane_remap_idempotency():
    ...
    # Ghost entity as returned by farfield.symmetry_plane (partial GhostCircularPlane token)
    ghost_from_farfield = GhostCircularPlane(name="symmetric", private_attribute_id="symmetric")
    ...
    assert errors is None, f"Expected no errors but got: {errors}"
    # Validator surfaces a deprecation warning naming the replaced surface
    assert len(warnings) == 1, warnings
    assert "farfield.symmetry_plane" in warnings[0]["msg"]
    assert "mySymmetry" in warnings[0]["msg"]
    ...
    # Ghost entity in entity_info is left as-is
    ghost_names = [
        g.name for g in validated.private_attribute_asset_cache.project_entity_info.ghost_entities
    ]
    assert "symmetric" in ghost_names
    assert "mySymmetry" not in ghost_names

    # Idempotency: revalidating the result is a no-op (entities unchanged)
    validated2, errors2, warnings2 = validate_model(
        params_as_dict=dump_wire_legal(validated),
        validated_by=ValidationCalledBy.LOCAL,
        root_item_type="SurfaceMesh",
        validation_level="All",
    )
    ...
    assert warnings2 == []
    sym_bc2 = next(m for m in validated2.models if isinstance(m, SymmetryPlane))
    assert [e.name for e in sym_bc2.entities.stored_entities] == ["mySymmetry"]
    ...
    with SI_unit_system:
        params = SimulationParams(
            ...
            models=[
                Wall(entities=[surface_body]),
                SymmetryPlane(
                    entities=GhostCircularPlane(name="symmetric", private_attribute_id="symmetric")
                ),
            ],
            private_attribute_asset_cache=asset_cache,
        )
...
def test_udf_multi_patch_symmetry():
    """UDF + multiple y=0 faces: direct refs pass, farfield.symmetry_plane expands to all."""
    surface_sym_a = Surface(
        name="sym_a",
        private_attribute_id="sym_a",
        private_attributes=SurfacePrivateAttributes(bounding_box=[[-1, 0, 0], [0, 0, 1]]),
    )
    surface_sym_b = Surface(
        name="sym_b",
        private_attribute_id="sym_b",
    ...
    # farfield.symmetry_plane (ghost) with multiple y=0 faces expands to all of them
    with SI_unit_system:
        defaults = MeshingDefaults(
            planar_face_tolerance=1e-4,
            geometry_accuracy=1e-5,
    ...
    # Deprecation warning lists both expanded surfaces
    assert errors is None, f"Expected no errors but got: {errors}"
    assert len(warnings) == 1, warnings
    assert "farfield.symmetry_plane" in warnings[0]["msg"]
    assert "sym_a" in warnings[0]["msg"] and "sym_b" in warnings[0]["msg"]

    # Ghost replaced; both expanded user surfaces present
    sym_bc = next(m for m in validated.models if isinstance(m, SymmetryPlane))
    names = [e.name for e in sym_bc.entities.stored_entities]
    assert "symmetric" not in names
    assert set(names) == {"sym_a", "sym_b"}
...
def test_mirror_unknown_boundary_still_raises_error():
        ...
        params = SimulationParams(
            models=[
                ...
                Wall(
                    entities=[
                        mirrored_front,
                        Surface(name="typo_surface", private_attribute_id="typo_surface"),
                    ]
                ),
            ],
            private_attribute_asset_cache=asset_cache,
    ...
    # Deliberately NOT wire-legalized: typo_surface must stay unknown.
...
def test_automated_farfield_with_custom_zones():
    ...
    _, errors, _ = validate_model(
        params_as_dict=dump_wire_legal(params_no_enclosed),
        validated_by=ValidationCalledBy.LOCAL,
        root_item_type="SurfaceMesh",
        validation_level="VolumeMesh",
    )
    ...
    with SI_unit_system:
        params_no_cv = SimulationParams(
            meshing=MeshingParams(
                ...
                volume_zones=[
                    AutomatedFarfield(
                        enclosed_entities=[Surface(name="face1", private_attribute_id="face1")],
                    ),
                ],
            ),
            private_attribute_asset_cache=AssetCache(use_inhouse_mesher=True),
    ...
    _, errors, _ = validate_model(
        params_as_dict=dump_wire_legal(params_no_cv),
        validated_by=ValidationCalledBy.LOCAL,
        root_item_type="SurfaceMesh",
        validation_level="VolumeMesh",
    )
