...
from flow360_schema.models.simulation.framework.updater_utils import compare_values
...
def test_default_seed_constructs_without_models_and_outputs():
    """Invariant the geometry conversion pipeline relies on for the in-house
    tessellator: the default seed minus its models and outputs sections (both
    may carry selector tokens the lenient constructor cannot resolve) must be
    directly constructible."""
    from flow360_schema.models.simulation.services import get_default_params

    import flow360

    seed = get_default_params("SI", "m", "Geometry")
    seed.pop("models", None)
    seed.pop("outputs", None)
    params = flow360.SimulationParams.load_lenient(seed)
    assert params.meshing is not None
