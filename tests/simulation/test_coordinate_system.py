...
def test_coordinate_system_transformation_matches_legacy_transformation():
    with u.SI_unit_system:
        ...
        coordinate_transform_args = {
            "reference_point": (1.0, 2.0, 3.0) * u.m,
            "axis_of_rotation": (0, 0, 1),
            "angle_of_rotation": 90 * u.deg,
            "scale": (2.0, 3.0, 4.0),
            "translation": (5.0, 6.0, 7.0) * u.m,
        }
        transformation_matrix = Transformation(**legacy_transform_args).get_transformation_matrix()
        coordinate_matrix = CoordinateSystem(
            name="vehicle_frame", **coordinate_transform_args
