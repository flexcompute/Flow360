...
from flow360_schema.models.simulation.meshing_param.meshing_specs import MeshingDefaults
...
from flow360_schema.models.simulation.models.volume_models import PorousMedium
...
def test_draft_end_to_end_selector_and_draft_entity_roundtrip(mock_surface_mesh, monkeypatch):
    ...
    with create_draft(new_run_from=mock_surface_mesh) as draft:
        ...
        # The raw params go up as-is; validation happens against the uploaded document
        # (the backend roundtrip below), matching the submit path.
        ...
        # Success criteria (d): the uploaded wire is COMPACT everywhere — every
        # stored_entities item outside the asset cache is a {"type", "ids"}
        # reference group, never an inline entity definition.
        def _assert_compact_wire(node, path):
                    ...
                    if key == "stored_entities" and isinstance(value, list):
                        for index, item in enumerate(value):
                            assert isinstance(item, dict) and set(item) == {
                                "type",
                                "ids",
                            }, f"inline entity on the wire at {path}.stored_entities[{index}]: {item}"
                    _assert_compact_wire(value, f"{path}.{key}")
            elif isinstance(node, list):
                for index, item in enumerate(node):
                    _assert_compact_wire(item, f"{path}[{index}]")

        _assert_compact_wire(uploaded_dict, "$")
