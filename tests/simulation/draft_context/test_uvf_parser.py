...
from flow360.component.simulation.measurement.uvf_parser import (
    _find_section,
    extract_face_triangles,
...
class TestExtractFaceVerticesIndexed:
    ...
    def test_triangles_are_grouped_by_requested_face_order(self, indexed_scene):
        manifest, bin_data_map = indexed_scene
        triangles = extract_face_triangles(manifest, ["face_b", "face_a"], bin_data_map)

        assert list(triangles) == ["face_b", "face_a"]
        assert triangles["face_b"].shape == (1, 3, 3)
        assert triangles["face_a"].shape == (2, 3, 3)
        np.testing.assert_array_equal(
            triangles["face_b"][0],
            np.array([[0, 0, 0], [1, 0, 0], [0.5, 0.5, 1]], dtype=np.float32),
        )
...
class TestExtractFaceVerticesUnindexed:
    ...
    def test_triangles_flatten_to_existing_vertex_result(self, unindexed_scene):
        manifest, bin_data_map = unindexed_scene
        triangles = extract_face_triangles(manifest, ["face_1"], bin_data_map)
        vertices = extract_face_vertices(manifest, ["face_1"], bin_data_map)

        assert triangles["face_1"].shape == (2, 3, 3)
        np.testing.assert_array_equal(triangles["face_1"].reshape(-1, 3), vertices)
