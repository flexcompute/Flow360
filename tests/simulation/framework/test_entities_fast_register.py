...
from flow360_schema.framework.param_utils import AssetCache, register_entity_list
from flow360_schema.framework.physical_dimensions import Length
from flow360_schema.models.entities.surface_entities import Surface
from flow360_schema.models.entities.volume_entities import Box, Cylinder, GenericVolume
from flow360_schema.models.simulation.simulation_params import _ParamModelBase
    ...
    def _populate_registry(self):
        ...
        for zone_name, zone_meta in self._get_meta_data()["zones"].items():
            ...
            known_frozen_hashes = self.internal_registry.fast_register(
                GenericVolume(
                    name=zone_name,
                    private_attribute_id=zone_name,
                    private_attribute_zone_boundary_names=all_my_boundaries,
                ),
                known_frozen_hashes,
            )
        # get interfaces
        ...
        for surface_name in self._get_meta_data()["surfaces"]:
            known_frozen_hashes = self.internal_registry.fast_register(
                Surface(
                    name=surface_name,
                    private_attribute_id=surface_name,
                    private_attribute_is_interface=surface_name in interfaces,
                ),
                known_frozen_hashes,
            )
