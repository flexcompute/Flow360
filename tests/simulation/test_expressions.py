...
def test_project_variables_serialization():
    ...
    from flow360_schema.testing import attach_wire_entity_info

    attach_wire_entity_info(params)
    ...
    ref_path = "ref/simulation_with_project_variables.json"
    generated = json.loads(params.model_dump_json(exclude_none=True))

    with open(ref_path, "r") as fh:
        ref_data = json.load(fh)

    # `version` is the flow360-schema package version stamped at dump time, so it goes stale
    # on every release bump. The ref keeps it for the updater path exercised by
    # test_project_variables_deserialization, but it is not what this test pins.
    generated.pop("version")
    ref_data.pop("version")
    assert generated == ref_data
