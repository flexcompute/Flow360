...
class TestNavierStokesLinearSolverTypes:
    ...
    def test_dict_without_type_name_defaults_to_linear_solver(self):
        """WebUI sends linear_solver dicts without type_name."""
        ns = NavierStokesSolver(linear_solver={"max_iterations": 20})
        assert isinstance(ns.linear_solver, LinearSolver)
        assert not isinstance(ns.linear_solver, KrylovLinearSolver)
        assert ns.linear_solver.max_iterations == 20

    def test_dict_without_type_name_empty(self):
        """WebUI may send an empty linear_solver dict."""
        ns = NavierStokesSolver(linear_solver={})
        assert isinstance(ns.linear_solver, LinearSolver)
        assert ns.linear_solver.max_iterations == 30
