...
def _minimal_simulation(models):
    # Register every inline surface in entity_info so the updater's
    # inline-to-reference conversion can resolve the assignments.
    boundaries = [
        dict(entity)
        for model in models
        for entity in (model.get("entities") or {}).get("stored_entities") or []
        if isinstance(entity, dict)
        and entity.get("private_attribute_entity_type_name") == "Surface"
    ]
    return {
        "version": "25.9.0",
        "private_attribute_asset_cache": {
            "project_entity_info": {"type_name": "SurfaceMeshEntityInfo", "boundaries": boundaries}
        },
        "unit_system": {"name": "SI"},
        "operating_condition": {
