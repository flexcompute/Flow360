...
def test_asset_group_help_does_not_import_simulation_summary(monkeypatch):
    ...
    _unload_modules(
        ...
        "flow360.exceptions",
        "flow360_schema.models.simulation.simulation_params",
    ...
    assert "flow360_schema.models.simulation.simulation_params" not in sys.modules
...
def test_measurement_namespace_is_exposed():
    import flow360 as fl  # pylint: disable=import-outside-toplevel,import-error

    assert "measure" in fl.__all__
    assert fl.measure.__name__ == "flow360.component.simulation.measurement"
    assert callable(fl.measure.oriented_bounding_box)
