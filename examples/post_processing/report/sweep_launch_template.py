...
######################################################################################################################
def make_run_params(draft, mesh_object, models):
    """
    Create the params object that contains all the run parameters.
    Needs the draft to get the list of surfaces.
    """
    with fl.SI_unit_system:
        params = fl.SimulationParams(
            ...
            outputs=[
                ...
                # draft.surfaces['*'] will select all the boundaries in the draft and export the
                # surface results. Regular expressions can be used to filter for certain boundaries.
                fl.SurfaceOutput(
...
def assign_boundary_conditions(draft, project):
    ...
    if project.project_tree.root.asset_type == "Geometry":
        ...
        models = [
            fl.Wall(
                surfaces=[
                    draft.surfaces["fluid/fuselage"],
                    # *_pylon will select all boundaries ending with _pylon
                    draft.surfaces["fluid/*_pylon"],
                    draft.surfaces["fluid/left_wing"],
                    draft.surfaces["fluid/right_wing"],
                    draft.surfaces["fluid/h_tail"],
                    draft.surfaces["fluid/v_tail"],
                ],
            ),
            fl.Freestream(surfaces=draft.surfaces["fluid/farfield"]),
        ]
    return models
...
def project_from_geometry():
    ...
    with fl.create_draft(
        new_run_from=project.geometry, face_grouping="faceName", edge_grouping="edgeName"
    ) as draft:
        mesh_params = create_mesh_params(draft)
...
def main():
    ...
    # The draft is created from the asset the sweep should start from; it must stay
    # active (inside the `with` block) while cases are submitted.
    if project.project_tree.root.asset_type == "Geometry":
        draft = fl.create_draft(
            new_run_from=project.geometry, face_grouping="faceName", edge_grouping="edgeName"
        )
    else:
        draft = fl.create_draft(new_run_from=vm)

    with draft:
        models = assign_boundary_conditions(draft, project)
        params = make_run_params(draft, vm, models)
