...
def simulation_params(draft, angle_of_attack):
...
with fl.create_draft(
    new_run_from=geo, face_grouping="groupName", edge_grouping="edgeName"
) as draft:
    for alpha in [0, 2, 4]:
        case = project.run_case(
            params=simulation_params(draft, alpha), name=f"Case for report, alpha={alpha}"
        )
        cases.append(case)
