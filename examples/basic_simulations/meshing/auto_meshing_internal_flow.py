...
with (
    fl.create_draft(
        new_run_from=geometry, face_grouping="faceName", edge_grouping="edgeId"
    ) as draft,
    fl.SI_unit_system,
):
        ...
        meshing=fl.MeshingParams(
            ...
            refinements=[
                fl.SurfaceRefinement(
                    name="sphere", max_edge_length=0.1, faces=[draft.surfaces["sphere"]]
                ),
                fl.SurfaceRefinement(
                    name="strut", max_edge_length=0.01, faces=[draft.surfaces["strut"]]
                ),
                fl.BoundaryLayer(
                    name="floor", first_layer_thickness=1e-5, faces=[draft.surfaces["floor"]]
                ),
                fl.PassiveSpacing(
                    name="adjacent2floor",
                    type="projected",
                    faces=[draft.surfaces["adjacent2floor"]],
                ),
                fl.PassiveSpacing(
                    name="ceiling", type="unchanged", faces=[draft.surfaces["ceiling"]]
                ),
            ],
        ),
    )
