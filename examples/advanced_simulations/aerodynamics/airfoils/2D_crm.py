...
with (
    fl.create_draft(
        new_run_from=geometry, face_grouping="faceName", edge_grouping="edgeName"
    ) as draft,
    fl.SI_unit_system,
):
    cylinders = [
        ...
        meshing=fl.MeshingParams(
            ...
            refinements=[
                ...
                fl.SurfaceRefinement(
                    name="wing", max_edge_length=0.74, faces=[draft.surfaces["wing"]]
                ),
                fl.SurfaceRefinement(
                    name="flap-slat",
                    max_edge_length=0.55,
                    faces=[draft.surfaces["flap"], draft.surfaces["slat"]],
                ),
                fl.SurfaceRefinement(
                    name="trailing",
                    max_edge_length=0.36,
                    faces=[
                        draft.surfaces["*Trailing"],
                    ],
                ),
                fl.SurfaceEdgeRefinement(
                    ...
                    edges=[
                        draft.edges["*trailingEdge"],
                        draft.edges["*leadingEdge"],
                    ],
                ),
                fl.SurfaceEdgeRefinement(
                    name="symmetry",
                    method=fl.ProjectAnisoSpacing(),
                    edges=[draft.edges["symmetry"]],
                ),
            ],
        ),
